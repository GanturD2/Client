...
CGraphicImage * CGrannyMaterial::__GetImagePointer(const char * fileName)
{
	...
	if (fileName_len > 2 && fileName[1] != ':')
	{
		...
		if (localFileName_len < sizeof(localFileName) - 1)
		{
			...
			return msl::inherit_cast<CGraphicImage *>(pResource);
		}
	}
...
bool CGrannyMaterial::CreateFromGrannyMaterialPointer(granny_material * pgrnMaterial)
{
	...
	if (pgrnMaterial)
	{
		...
		// Two-Side 렌더링이 필요한 지 검사
		{
			...
			granny_data_type_definition TwoSidedFieldType[] = {{GrannyInt32Member, "Two-sided"}, {GrannyEndMember}};
#if GrannyProductMinorVersion == 4
			granny_variant twoSideResult =
				GrannyFindMatchingMember(pgrnMaterial->ExtendedData.Type, pgrnMaterial->ExtendedData.Object, "Two-sided");
			...
			granny_variant twoSideResult;
			bool findMatchResult =
				GrannyFindMatchingMember(pgrnMaterial->ExtendedData.Type, pgrnMaterial->ExtendedData.Object, "Two-sided", &twoSideResult);
			if (nullptr != twoSideResult.Type && findMatchResult)
				GrannyConvertSingleObject(twoSideResult.Type, twoSideResult.Object, TwoSidedFieldType, &twoSided);
...
void CGrannyMaterial::__ApplySpecularRenderState()
{
	...
	STATEMANAGER.SetRenderState(D3DRS_TEXTUREFACTOR,
								D3DXCOLOR(g_fSpecularColor.r, g_fSpecularColor.g, g_fSpecularColor.b, __GetSpecularPower()));
...
{
	CResourceManager & rkResMgr = CResourceManager::Instance();
	auto * pImage = msl::inherit_cast<CGraphicImage *>(rkResMgr.GetResourcePointer(c_szSphereMapImageFileName));
...
void CGrannyMaterialPalette::SetMaterialImagePointer(const char * c_szImageName, CGraphicImage * pImage)
{
	...
	for (uint32_t i = 0; i < size; ++i)
	{
		...
		{
			auto * pkNewMtrl = new CGrannyMaterial;
