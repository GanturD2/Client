...
bool CGrannyModelInstance::Intersect(const D3DXMATRIX * c_pMatrix, float * /*pu*/, float * /*pv*/, float * pt)
{
...
void CGrannyModelInstance::GetBoundBox(D3DXVECTOR3 * vtMin, D3DXVECTOR3 * vtMax)
{
	...
	for (int m = 0; m < meshCount; ++m)
	{
		...
		for (int b = 0; b < pgrnMesh->BoneBindingCount; ++b)
		{
			...
			MakeBoundBox(&BoundBox, GrannyGetWorldPose4x4(__GetWorldPosePtr(), boneIndices[b]), rgrnBoneBinding.OBBMin,
