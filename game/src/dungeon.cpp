...
#ifdef ENABLE_D_NJGUILD
	if (m_pGuild != nullptr)
		m_pGuild->SetDungeon_for_Only_guild(nullptr);
#endif
...
#endif
#ifdef ENABLE_ACHIEVEMENT_SYSTEM
	dungeon_id = startTime_ = endTime_ = 0;
#endif
...
void CDungeon::QuitParty(LPPARTY pParty)
{
	...
	const TPartyMap::iterator it = m_map_pkParty.find(pParty); // @warme011 boom! crash!
...
{
	const dungeon_id_info* info = dynamic_cast<dungeon_id_info*>(event->info);
...
#ifdef ENABLE_DUNGEON_DEAD_TIME_RENEWAL
		if (m_bIsDeadEvent)
			deadEvent = event_create(dungeon_dead_event, info, PASSES_PER_SEC(m_lDeadTime));
#else
...
void CDungeon::IncPartyMember(LPPARTY pParty, LPCHARACTER ch)
{
	...
	const TPartyMap::iterator it = m_map_pkParty.find(pParty);
...
void CDungeon::JumpGuild(CGuild* pGuild, long lFromMapIndex, int x, int y)
{
	...
	const LPSECTREE_MAP& pMap = SECTREE_MANAGER::Instance().GetMap(lFromMapIndex);
...
void CDungeonManager::Destroy(CDungeon::IdType dungeon_id)
{
	...
	const long lMapIndex = pDungeon->m_lMapIndex;
	m_map_pkMapDungeon.erase(lMapIndex);

	const uint32_t server_timer_arg = lMapIndex;
...
{
	const TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
...
{
	const TAreaMap::iterator it_to = m_map_Area.find(pos_to);
	...
	const TAreaMap::iterator it_from = m_map_Area.find(pos_from);
	...
	const LPSECTREE_MAP& pkSectreeMap = SECTREE_MANAGER::Instance().GetMap(m_lMapIndex);
	...
	for (int i = 0; i < 100; i++)
	{
		const int dx = number(ai.sx, ai.ex);
		const int dy = number(ai.sy, ai.ey);
		const int tx = number(ai_to.sx, ai_to.ex);
		const int ty = number(ai_to.sy, ai_to.ey);
...
{
	const TAreaMap::iterator it = m_map_Area.find(pos);
...
uint32_t CDungeon::GetUniqueVid(const std::string& key)
{
	...
	const LPCHARACTER& ch = it->second;
	if (!ch)
		return 0;
...
#ifdef ENABLE_SUNG_MAHI_TOWER
LPCHARACTER CDungeon::SpawnMob(uint32_t vnum, int x, int y, int dir, bool isNomove)
#else
LPCHARACTER CDungeon::SpawnMob(uint32_t vnum, int x, int y, int dir)
#endif
{
	if (!vnum)
		return nullptr;
...
LPCHARACTER CDungeon::SpawnMob_ac_dir(uint32_t vnum, int x, int y, int dir)
{
	const LPSECTREE_MAP& pkSectreeMap = SECTREE_MANAGER::Instance().GetMap(m_lMapIndex);
	if (pkSectreeMap == nullptr)
	{
...
{
	constexpr int MOB_GOTO_VNUM = 20039;
...
LPCHARACTER CDungeon::SpawnGroup(uint32_t vnum, long x, long y, float radius, bool bAggressive, int count)
{
	const LPSECTREE_MAP& pkSectreeMap = SECTREE_MANAGER::Instance().GetMap(m_lMapIndex);
	if (pkSectreeMap == nullptr)
	{
	...
	const int iRadius = static_cast<int>(radius);

	const int sx = pkSectreeMap->m_setting.iBaseX + x - iRadius;
	const int sy = pkSectreeMap->m_setting.iBaseY + y - iRadius;
	const int ex = sx + iRadius;
	const int ey = sy + iRadius;
...
bool CDungeon::IsValidRegen(LPREGEN regen, size_t regen_id)
{
	...
	const LPREGEN& found = *it;
	return (found->id == regen_id);
}

void CDungeon::SpawnMoveGroup(uint32_t vnum, const char* pos_from, const char* pos_to, int count)
{
	const TAreaMap::iterator it_to = m_map_Area.find(pos_to);
	...
	while (count--)
	{
		const int tx = number(ai_to.sx, ai_to.ex) + pkSectreeMap->m_setting.iBaseX;
		const int ty = number(ai_to.sy, ai_to.ey) + pkSectreeMap->m_setting.iBaseY;
...
struct FCountMonster
{
	...
	void operator()(LPENTITY ent)
	{
		if (ent && ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (ch && (ch->IsMonster() || ch->IsStone())) // @fixme171 previously (!ch->IsPC())
				n++;
		}
	}
};
...
namespace
{
	...
	struct FSyschat
	{
		FSyschat(const char* psz) : m_psz(psz)
		{
		}
		...
		void operator() (LPENTITY ent)
		{
			if (ent && ent->IsType(ENTITY_CHARACTER))
			{
				LPCHARACTER ch = dynamic_cast<LPCHARACTER>(ent);
				if (ch)
					ch->ChatPacket(CHAT_TYPE_INFO, "%s", m_psz);
			}
		}
...
void CDungeon::Notice(const char* msg)
{
	...
	const LPSECTREE_MAP& pMap = SECTREE_MANAGER::Instance().GetMap(m_lMapIndex);
...
#ifdef ENABLE_NEW_CHAT_DUNGEON
void CDungeon::Syschat(const char* msg)
{
	sys_log(0, "XXX Dungeon Syschat %p %s", this, msg);
	LPSECTREE_MAP pMap = SECTREE_MANAGER::Instance().GetMap(m_lMapIndex);
...
{
	sys_log(0, "XXX Dungeon BigNotice %p %s", this, msg);
	LPSECTREE_MAP pMap = SECTREE_MANAGER::Instance().GetMap(m_lMapIndex);
...
namespace
{
	struct FZodiacTime
	{
		...
		FZodiacTime(uint8_t c, uint8_t n, int t)
...
void CDungeon::ZodiacTime(uint8_t currentfloor, uint8_t nextfloor, int time)
{
	LPSECTREE_MAP pMap = SECTREE_MANAGER::Instance().GetMap(m_lMapIndex);
	if (!pMap)
	{
	...
	void operator () (LPENTITY ent)
	{
		if (ent && ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (ch && ch->IsPC())
			{
				...
				// Instead of putting the current map index, we put the starting map index.
...
{
	const ItemGroupMap::iterator it = m_map_ItemGroup.find(group_name);
...
#ifdef ENABLE_ACHIEVEMENT_SYSTEM
void CDungeon::FinishDungeon()
{
	for (const auto& character : m_set_pkCharacter)
	{
		if (character) {
			CAchievementSystem::Instance().OnFinishDungeon(
				character, dungeon_id, GetFinishTime(), m_set_pkCharacter.size());
		}
	}
}
#endif
...
LPCHARACTER CDungeon::SpawnGroupLevelRange(uint32_t vnum, long x, long y, float radius, bool bAggressive, int count, uint8_t min_level, uint8_t max_level, bool bIncreaseHP, bool bIncreaseDamage)
{
	const LPSECTREE_MAP& pkSectreeMap = SECTREE_MANAGER::Instance().GetMap(m_lMapIndex);
	if (pkSectreeMap == nullptr)
	{
	...
	while (count--)
	{
		LPCHARACTER chLeader = CHARACTER_MANAGER::Instance().SpawnGroupLevelRange(vnum, m_lMapIndex, sx, sy, ex, ey, min_level, max_level,
			bIncreaseHP, bIncreaseDamage, nullptr, bAggressive, this);
		if (chLeader && !ch)
			ch = chLeader;
	}
...
#ifdef ENABLE_DUNGEON_RENEWAL
void CDungeon::RegisterParticipant(const LPCHARACTER& pkChar)
{
	if (!pkChar)
		return;
	...
	const auto it = m_Participants.find(dwPlayerID);
	if (it == m_Participants.end())
		m_Participants.insert(it, std::pair<uint32_t, std::string>(dwPlayerID, szPlayerName));
}

bool CDungeon::IsParticipantRegistered(uint32_t dwPlayerID)
{
	const auto it = m_Participants.find(dwPlayerID);
	if (it != m_Participants.end())
		return true;
...
void CDungeon::ClearParticipants() noexcept
{
	m_Participants.clear();
}
#endif
...
{
	SUpdateMastHP(LPCHARACTER Mast, bool wood = false) : m_Mast(Mast), woodBuff(wood) {}
	...
	void operator () (LPENTITY ent)
	{
		if (ent && ent->IsType(ENTITY_CHARACTER))
		{
			...
			{
				if (woodBuff)
					m_Mast->SetHP(MINMAX(0, m_Mast->GetHP() + 50000, m_Mast->GetMaxHP()));
...
void CDungeon::UpdateMastHP(bool woodBuff)
{
	LPSECTREE_MAP pMap = SECTREE_MANAGER::Instance().GetMap(m_lMapIndex);
	if (!pMap)
	{
	...
	SUpdateMastHP f(GetMast(), woodBuff ? true : false);
