...
{
	{
		...
		int cachedBattlePassID = GetNormalBattlePassID(static_cast<uint8_t>(BATTLEPASS_NORMAL));
		bool findActiveBattlePass = false;
		if (cachedBattlePassID)
			findActiveBattlePass = true;

		auto cachedStartTime = GetNormalBattlePassStartTime(static_cast<uint8_t>(BATTLEPASS_NORMAL));
		auto cachedEndTime = GetNormalBattlePassEndTime(static_cast<uint8_t>(BATTLEPASS_NORMAL));
#else
		bool findActiveBattlePass = false;
	...
	{
#ifdef ENABLE_EVENT_MANAGER
		int cachedBattlePassID = GetNormalBattlePassID(static_cast<uint8_t>(BATTLEPASS_PREMIUM));
		bool findActiveBattlePass = false;
		if (cachedBattlePassID)
			findActiveBattlePass = true;

		auto cachedStartTime = GetNormalBattlePassStartTime(static_cast<uint8_t>(BATTLEPASS_PREMIUM));
		auto cachedEndTime = GetNormalBattlePassEndTime(static_cast<uint8_t>(BATTLEPASS_PREMIUM));
#else
		bool findActiveBattlePass = false;
		int cachedBattlePassID, cachedStartTime, cachedEndTime;
	...
	{
#ifdef ENABLE_EVENT_MANAGER
		int cachedBattlePassID = GetNormalBattlePassID(static_cast<uint8_t>(BATTLEPASS_EVENT));
		bool findActiveBattlePass = false;
		if (cachedBattlePassID)
			findActiveBattlePass = true;

		auto cachedStartTime = GetNormalBattlePassStartTime(static_cast<uint8_t>(BATTLEPASS_EVENT));
		auto cachedEndTime = GetNormalBattlePassEndTime(static_cast<uint8_t>(BATTLEPASS_EVENT));
#else
		bool findActiveBattlePass = false;
		int cachedBattlePassID, cachedStartTime, cachedEndTime;
...
uint8_t CBattlePassManager::GetMissionIndex(uint8_t bBattlePassType, uint8_t bMissionType, uint32_t dwCondition)
{
...
uint8_t CBattlePassManager::GetMissionTypeByIndex(uint8_t bBattlePassType, uint32_t dwIndex)
{
	if (bBattlePassType == 1)
	{
		TMapNormalBattlePassName::iterator it_normal = m_map_normal_battle_pass_name.find(m_dwActiveNormalBattlePassID);
...
std::string CBattlePassManager::GetNormalBattlePassNameByID(uint8_t bID)
{
...
std::string CBattlePassManager::GetPremiumBattlePassNameByID(uint8_t bID)
{
...
std::string CBattlePassManager::GetEventBattlePassNameByID(uint8_t bID)
{
...
void CBattlePassManager::BattlePassRequestOpen(LPCHARACTER pkChar, bool bAfterOpen)
{
	...
	
	{
		...
		// --- GENERAL INFOS
#ifdef ENABLE_EVENT_MANAGER
		const int iID = GetNormalBattlePassID(static_cast<uint8_t>(BATTLEPASS_NORMAL));
		if (iID > 0)
		{
			const auto iStartTime = GetNormalBattlePassStartTime(static_cast<uint8_t>(BATTLEPASS_NORMAL));
			const auto iEndTime = GetNormalBattlePassEndTime(static_cast<uint8_t>(BATTLEPASS_NORMAL));
			...
			packet.dwBattlePassID = iID;
			packet.dwBattlePassStartTime = iStartTime;
			packet.dwBattlePassEndTime = iEndTime;
...
#ifdef ENABLE_EVENT_MANAGER
		const int iID = GetNormalBattlePassID(static_cast<uint8_t>(BATTLEPASS_PREMIUM));
		if (iID > 0)
		{
			const auto iStartTime = GetNormalBattlePassStartTime(static_cast<uint8_t>(BATTLEPASS_PREMIUM));
			const auto iEndTime = GetNormalBattlePassEndTime(static_cast<uint8_t>(BATTLEPASS_PREMIUM));
...
#ifdef ENABLE_EVENT_MANAGER
		const int iID = GetNormalBattlePassID(static_cast<uint8_t>(BATTLEPASS_EVENT));
		if (iID > 0)
		{
			const auto iStartTime = GetNormalBattlePassStartTime(static_cast<uint8_t>(BATTLEPASS_EVENT));
			const auto iEndTime = GetNormalBattlePassEndTime(static_cast<uint8_t>(BATTLEPASS_EVENT));
...
void CBattlePassManager::BattlePassRewardMission(LPCHARACTER pkChar, uint8_t bBattlePassType, uint32_t bBattlePassId, uint8_t bMissionIndex)
{
	if (!pkChar)
		return;
...
void CBattlePassManager::BattlePassRequestReward(LPCHARACTER pkChar, uint8_t bBattlePassType)
{
	if (!pkChar)
		return;
	...
	if (bBattlePassType == 1)
	{
		...
		for (int i = 0; i < missionInfo.size(); i++) {
			if (!pkChar->IsExtBattlePassCompletedMission(bBattlePassType, missionInfo[i].bMissionIndex, missionInfo[i].bMissionType)) {
				pkChar->ChatPacket(CHAT_TYPE_INFO, "[LS;1930]");
				return;
			}
		}
	}
	if (bBattlePassType == 2)
	{
		...
		for (int i = 0; i < missionInfo.size(); i++) {
			if (!pkChar->IsExtBattlePassCompletedMission(bBattlePassType, missionInfo[i].bMissionIndex, missionInfo[i].bMissionType)) {
				pkChar->ChatPacket(CHAT_TYPE_INFO, "[LS;1931]");
				return;
			}
		}
	}
	if (bBattlePassType == 3)
	{
		...
		for (int i = 0; i < missionInfo.size(); i++) {
			if (!pkChar->IsExtBattlePassCompletedMission(bBattlePassType, missionInfo[i].bMissionIndex, missionInfo[i].bMissionType)) {
				pkChar->ChatPacket(CHAT_TYPE_INFO, "[LS;1932]");
				return;
			}
		}
	}
	...
	if (bHasRecived != 0) {
		pkChar->ChatPacket(CHAT_TYPE_INFO, "[LS;1933]");
		return;
	}
...
void CBattlePassManager::BattlePassReward(LPCHARACTER pkChar, uint8_t bBattlePassType, uint8_t bBattlePassID)
{
	if (bBattlePassType == 1)
	{
		TMapNormalBattlePassName::iterator it = m_map_normal_battle_pass_name.find(bBattlePassID);
...
bool CBattlePassManager::BattlePassMissionGetInfo(uint8_t bBattlePassType, uint8_t bMissionIndex, uint8_t bBattlePassId, uint8_t bMissionType, uint32_t* dwFirstInfo, uint32_t* dwSecondInfo)
{
	if (bBattlePassType == 1)
	{
		TMapNormalBattlePassName::iterator it = m_map_normal_battle_pass_name.find(bBattlePassId);
