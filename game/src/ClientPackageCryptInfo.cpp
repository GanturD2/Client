...
bool CClientPackageCryptInfo::LoadPackageCryptFile( const char* pCryptFile )
{
...

	{
		...
		else
		{
			...
			fread( &m_vecPackageCryptKeys[nCurKeySize + sizeof(int)], sizeof(uint8_t), iCryptKeySize, fp );
...
void CClientPackageCryptInfo::GetPackageCryptKeys( uint8_t** ppData, int& iDataSize )
{
...
bool CClientPackageCryptInfo::GetRelatedMapSDBStreams(const char* pMapName, uint8_t** ppData, int& iDataSize )
{
