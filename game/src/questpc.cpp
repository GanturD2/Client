...
namespace quest
{
	...
	int PC::GetMoveTargetX()
	{
		return 0; //missing
	}

	int PC::GetMoveTargetY()
	{
		return 0; //missing
	}
#endif
	...
	void PC::SendQuestInfoPacket()
	{
		...
		qi.size = sizeof(TPacketGCQuestInfo);
	...
	void PC::EndRunning()
	{
		...
		// fixme478
		if (CQuestManager::Instance().GetCurrentItem() != nullptr)
	...
	void PC::AutoGiveItem(uint32_t dwVnum, int count)
	{
		m_vRewardData.emplace_back(RewardData(RewardData::REWARD_TYPE_ITEM, dwVnum, count));
	}
