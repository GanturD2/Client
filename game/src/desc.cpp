...
#ifndef ENABLE_HAND_SHAKE_LIMIT
...
void DESC::SendHandshake(uint32_t dwCurTime, long lNewDelta)
{
...
bool DESC::HandshakeProcess(uint32_t dwTime, long lDelta, bool bInfiniteRetry)
{
...
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
	pack_chat.bLanguage = GetLanguage();
#endif
...
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
void DESC::ChatPacket(uint8_t bType, uint8_t bEmpire, uint8_t bLanguage, const char* format, ...)
{
	char chatbuf[CHAT_MAX_LEN + 1];
	va_list args;
