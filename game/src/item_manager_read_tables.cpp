		...
		else
		{
			...
			for (int k = 1; k < 1024; ++k) // @fixme148 256 -> 1024
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					origin_info.vnum = iVnum;
					origin_info.is_mob = false;
					m_itemOriginMap[addVnum].emplace_back(origin_info);
#endif
...
bool ITEM_MANAGER::ReadMonsterDropItemGroup(const char * c_pszFileName)
{
	...
	for (uint32_t i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		...
		if (strType == "kill")
		{
			...
			for (int k = 1; k < 256; ++k)
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					TWikiItemOriginInfo origin_info{};
					origin_info.vnum = iMobVnum;
					origin_info.is_mob = true;
					m_itemOriginMap[currVnum].emplace_back(origin_info);

					TWikiMobDropInfo mob_drop{};
					mob_drop.vnum = dwVnum;
					mob_drop.count = iCount;
					CMobManager::Instance().GetMobWikiInfo(iMobVnum).emplace_back(mob_drop);
#endif
					continue;
				}
		...
		else if ( strType == "index" )
		{
			...
			m_map_pkMapIndexItemGroup.insert(std::map<uint32_t, CMapIndexItemGroup*>::value_type(iMobVnum, pkMapIndexItemGroup));
		}
#endif
...
bool ITEM_MANAGER::ReloadSpecialItemGroup(const char* c_pszFileName)
{
	...
	{
		for (itertype(m_map_pkSpecialItemGroup) it = m_map_pkSpecialItemGroup.begin();
	...
	{
		for (itertype(m_map_pkQuestItemGroup) it = m_map_pkQuestItemGroup.begin();
	...
	{
		for (itertype(m_map_pkSpecialAttrGroup) it = m_map_pkSpecialAttrGroup.begin();
