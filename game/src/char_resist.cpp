...
void CHARACTER::AttackedByFire(const LPCHARACTER& pkAttacker, int amount, int count)
{
	if (m_pkFireEvent)
		return;

#ifdef ENABLE_MONSTER_BACK
	if (GetRaceNum() >= EMonsterBackEvent::ATTENDANCE_VNUM_BERAN && GetRaceNum() <= EMonsterBackEvent::ATTENDANCE_VNUM_RED_DRAGON)
		return;
#endif
...
bool CHARACTER::IsImmune(uint32_t dwImmuneFlag)
{
	...
	{
		int immune_pct = IMMUNE_PCT;
		int	percent = number(1, 100);

		if (percent <= immune_pct)	// Immune-Rate
		{
			if (test_server && IsPC())
