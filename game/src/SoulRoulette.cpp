...
bool CSoulRoulette::ReadRouletteData(bool NoMoreItem)
{
	...
	auto loader = new CGroupTextParseTreeLoader();
	...
	for (int i = 1; i <= ROULETTE_ITEM_MAX && i <= RowCount; i++) {
		...
		v_RouletteItem.emplace_back(new CSoulRoulette::SRoulette(vnum, count, chance));
	}

	return true;
}

void CSoulRoulette::SendPacket(uint8_t option, int arg1, int arg2)
{
	if (!ch->GetDesc())
		return;
...
void CSoulRoulette::TurnWheel()
{
	...
	if (ch->GetSoulPoint() < RouletteSoul) {
		ch->ChatPacket(CHAT_TYPE_INFO, "<Soul Roulette> You need soul.");
		return;
	}
...
{
	const uint8_t Chance = GetChance();
	...
	if (it != v_RouletteItem.end()) {
		const uint8_t MinChance = (*it)->chance;
...
{
	const uint32_t GiftVnum = GetGiftVnum();
...
void CSoulRoulette::StateError(uint8_t option, LPCHARACTER ch)
{
	...
	switch (option)
	{
		case CSoulRoulette::Error::LOAD: // from mysql
		{
			...
			auto pMsg1(DBManager::Instance().DirectQuery(szQuery));
			...
			while ((row = mysql_fetch_row(pMsg1->Get()->pSQLResult))) {
				uint32_t vnum; uint8_t count;
				str_to_number(vnum, row[0]);
				str_to_number(count, row[1]);
				v_Err.emplace_back(new CSoulRoulette::ErrorData(vnum, count, row[2]));
			}

			auto pMsg2(DBManager::Instance().DirectQuery("DELETE FROM log.%s WHERE state = 'ERROR'", mTableName));
			break;
		}
		...
		case CSoulRoulette::Error::ADD: // from game
			if (ch) {
				...
				if (Roulette)
					v_Err.emplace_back(new CSoulRoulette::ErrorData(Roulette->GetGiftVnum(), Roulette->GetGiftCount(), ch->GetName()));
			}
			break;
...
void CSoulRoulette::SetGift(const uint32_t vnum, const uint8_t count)
{
	gift_vnum = vnum;
	gift_count = count;
}

uint8_t CSoulRoulette::GetChance() const
{
	const uint16_t TurnCount = GetTurnCount();
...
uint32_t CSoulRoulette::GetGiftVnum() const
{
	return gift_vnum;
}

uint8_t CSoulRoulette::GetGiftCount() const
{
	return gift_count;
}

uint16_t CSoulRoulette::GetTurnCount() const
{
	return turn_count;
}
#endif
