...
ACMD(do_inputall)
{
	ch->ChatPacket(CHAT_TYPE_INFO, "[LS;916]");
}
...
ACMD(do_check_flag);
ACMD(do_switchbot);
ACMD(do_strip) { ch->StripPowers(); }	//@custom025
/*ACMD(do_pvp_set);
ACMD(do_gm_set);*/
ACMD(do_ds_qualify);
#ifdef ENABLE_12ZI
...
struct command_info cmd_info[] =
{
	...
	{ "give",		do_give_item,		0,			POS_DEAD,	GM_IMPLEMENTOR		},
...
//	{ "몬스터보여줘",	do_monsterlog,		0,			POS_DEAD,	GM_LOW_WIZARD	},//@fixme105
	...
	{ "con+",			do_stat_plus_amount,		POINT_HT,			POS_DEAD,		SET_STATES_PRIVILEGES },
	{ "int+",			do_stat_plus_amount,		POINT_IQ,			POS_DEAD,		SET_STATES_PRIVILEGES },
	{ "str+",			do_stat_plus_amount,		POINT_ST,			POS_DEAD,		SET_STATES_PRIVILEGES },
	{ "dex+",			do_stat_plus_amount,		POINT_DX,			POS_DEAD,		SET_STATES_PRIVILEGES },
#else
...
#endif
#endif
	{ "skillup2",					do_skillup2,				0,					POS_DEAD,		GM_PLAYER },
#ifdef ENABLE_AFFECT_POLYMORPH_REMOVE
...
#endif
	{ "check_flag",					do_check_flag,				0,					POS_DEAD,		GM_PLAYER },
	{ "switchbot",					do_switchbot,				0,					POS_DEAD,		GM_PLAYER },
	{ "strip",						do_strip,					0,					POS_DEAD,		GM_IMPLEMENTOR },
/*	{ "pvp_set",					do_pvp_set,					0,					POS_DEAD,		GM_IMPLEMENTOR },
	{ "gm_set",						do_gm_set,					0,					POS_DEAD,		GM_IMPLEMENTOR },*/
	{ "ds_qualify",					do_ds_qualify,				0,					POS_DEAD,		GM_IMPLEMENTOR },
	...
	{ "shop_manage", do_shop_manage, 0, POS_DEAD, GM_PLAYER },
#	ifdef ENABLE_OPEN_SHOP_WITHOUT_BAG
	{ "shop_open", do_shop_open, 0, POS_DEAD, GM_PLAYER },
#	endif
#	ifdef ENABLE_OPEN_SHOP_WITH_PASSWORD
	{ "click_shop_editor", do_click_shop_editor, 0, POS_DEAD, GM_PLAYER },
	{ "shop_password", do_shop_password, 0, POS_DEAD, GM_PLAYER },
#	endif
#endif
#ifdef ENABLE_BATTLE_FIELD
	{ "restart_immediate", do_restart, SCMD_RESTART_BATTLE, POS_DEAD, GM_PLAYER },
	{ "battle_force_close", do_battle_force_close, 0, POS_DEAD, GM_IMPLEMENTOR },
	{ "battle_force_open", do_battle_force_open, 0, POS_DEAD, GM_IMPLEMENTOR },
	{ "battle_set_event", do_battle_set_event, 0, POS_DEAD, GM_IMPLEMENTOR },
	{ "open_battle_ui", do_open_battle_ui, 0, POS_DEAD, GM_PLAYER },
	{ "goto_battle", do_goto_battle, 0, POS_DEAD, GM_PLAYER },
	{ "exit_battle_field", do_exit_battle_field, 0, POS_DEAD, GM_PLAYER },
	{ "exit_battle_field_on_dead", do_exit_battle_field_on_dead, 0, POS_DEAD, GM_PLAYER },
#endif
#ifdef ENABLE_ATTR_6TH_7TH_EXTEND
	{ "state_attr", do_state_attr, 0, POS_DEAD, GM_LOW_WIZARD },
#endif
#ifdef ENABLE_PASSIVE_ATTR
	{ "state_party", do_state_party, 0, POS_DEAD, GM_LOW_WIZARD },
#endif
	...
	{ "state_sungma",		do_state_sungma, 0, POS_DEAD, GM_LOW_WIZARD },
#endif
...
#ifdef ENABLE_TICKET_SYSTEM
	{ "delete_tickets_closed",	do_delete_tickets_closed,	0,	POS_DEAD,	GM_GOD },
#endif
...
#endif
#ifdef ENABLE_MINI_GAME_BNW
	{ "bnw_event",	do_bnw_event,	0,	POS_DEAD,	GM_IMPLEMENTOR },
#endif
#ifdef ENABLE_MINI_GAME_FINDM
	{ "find_m_event", do_find_m_event, 0, POS_DEAD, GM_IMPLEMENTOR },
#endif
#ifdef ENABLE_MINI_GAME_YUTNORI
	{ "yut_nori_event",	do_yut_nori_event,	0,	POS_DEAD,	GM_PLAYER },
#endif
...
#endif
#ifdef ENABLE_ACHIEVEMENT_SYSTEM
	{ "force_finish_achievement", do_force_finish_achievement, 0, POS_DEAD, GM_IMPLEMENTOR },
#endif
#ifdef ENABLE_BATTLE_PASS_SYSTEM
...
#ifdef ENABLE_ADDITIONAL_EQUIPMENT_PAGE
	{ "additional_equipment",			do_additional_equipment,	0,			POS_DEAD,		GM_PLAYER },
#endif
...
#ifdef ENABLE_WORLD_LOTTERY_SYSTEM
	{ "lottery_open", do_lottery_open, 0, POS_DEAD, GM_PLAYER },
#endif

#ifdef ENABLE_DS_REFINE_WINDOW
	{ "refine_open", do_open_refine_ds, 0, POS_DEAD, GM_PLAYER },
#endif
...
extern void interpret_command(LPCHARACTER ch, const char * argument, size_t len)
{
	...
	if (ch->GetPosition() < cmd_info[icmd].minimum_position)
	{
		...
		{
			case POS_MOUNTING:
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;917]");
				break;

			case POS_DEAD:
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;918]");
				break;

			case POS_SLEEPING:
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;919]");
				break;

			case POS_RESTING:
			case POS_SITTING:
				ch->ChatPacket(CHAT_TYPE_INFO, "[LS;920]");
				break;
