...
bool CSpeedServerEmpireExp::Initialize (uint8_t e)
{
	...
	for (int i = 1; i < 6; i++)
	{
		wday_exp_table[i].emplace_back (HME (18, 0, 50));
		wday_exp_table[i].emplace_back (HME (24, 0, 100));
	}

	wday_exp_table[0].emplace_back (HME (18, 0, 100));
	wday_exp_table[0].emplace_back (HME (24, 0, 150));
	wday_exp_table[6].emplace_back (HME (18, 0, 100));
	wday_exp_table[6].emplace_back (HME (24, 0, 150));
...
bool CSpeedServerEmpireExp::LoadWdayExpTable(int wday, char *str)
{
	...
	while (p != nullptr)
	{
		...
		lst.emplace_back (HME (hour, min, exp));
		p = strtok (n, ";");
		n = strtok (nullptr, ";");
	}
	return true;
...
bool CSpeedServerEmpireExp::WriteExpTable()
{
	...
	for (int i = 0; i < 7; i++)
	{
		...
		for (std::list <HME>::iterator it = wday_exp_table[i].begin(); it != wday_exp_table[i].end(); ++it)	//@fixme541
		{
	...
	for (std::map <Date, std::list <HME> >::iterator holi_it = holiday_map.begin(); holi_it != holiday_map.end(); ++holi_it)	//@fixme541
	{
		fprintf (fp, "HOLIDAY %d.%d.%d", holi_it->first.year + 1900, holi_it->first.mon + 1, holi_it->first.day);
		for (std::list <HME>::iterator it = holi_it->second.begin(); it != holi_it->second.end(); ++it)	//@fixme541
		{
			fprintf (fp, " %d:%d %d;", it->hour, it->min, it->exp);
		}
		fprintf(fp, "\n");
...
std::list <HME>& CSpeedServerManager::GetWdayExpTableOfEmpire(uint8_t empire, int wday)
{
...
void CSpeedServerManager::SetWdayExpTableOfEmpire (uint8_t empire, int wday, HME hme)
{
...
{
	wday_exp_table[wday].emplace_back (hme);
	WriteExpTable();
}

void CSpeedServerManager::InitWdayExpTableOfEmpire (uint8_t empire, int wday)
{
...
HME CSpeedServerManager::GetCurrentExpPrivOfEmpire (uint8_t empire, int &duration, bool &is_change)
{
...
HME CSpeedServerEmpireExp::GetCurrentExpPriv(int &duration, bool &is_change)
{
	...
	if (holi_it != holiday_map.end())
	{
		for (std::list <HME>::iterator it = holi_it->second.begin();
				it != wday_exp_table[datetime->tm_wday].end(); ++it)	//@fixme541
		{
