...
{
	Initialize();
	m_pPackageCrypt = M2_NEW CClientPackageCryptInfo;
}
...
struct name_with_desc_func
{
	...
	bool operator () (LPDESC d)
	{
		...
		return d->GetCharacter() && tolowerlocal(std::string(m_name)) == tolowerlocal(std::string(d->GetCharacter()->GetName()));	//@fixme419
	}
};
