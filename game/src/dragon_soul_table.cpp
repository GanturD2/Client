...
bool DragonSoulTable::ReadBasicApplys()
{
	...
	for (uint i = 0; i < m_vecDragonSoulNames.size(); i++)
	{
		...
		// BasicApply Group starts with Key 1.
		for (int j = 1; j <= n; j++)
		{
			std::stringstream ss;
			ss << j;
	...
	else
	{
		for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
		{
			for (int j = 0; j < DRAGON_SOUL_STEP_MAX - 1; j++)
			{
				...
				if (DRAGON_SOUL_STEP_MAX != vec_probs.size())	//@fixme483
				{
					sys_err("In %s group of RefineStepTables, probability list size is not %d -> %d.",
						m_vecDragonSoulNames[i].c_str(), DRAGON_SOUL_STEP_MAX, vec_probs.size());	//@fixme483
					return false;
				}
...
uint8_t DragonSoulTable::GetBasicApplyCount(uint8_t iType)
{
	...
	return static_cast<uint8_t>(vec_Applys.size());
}

bool DragonSoulTable::GetBasicApplyValue(uint8_t iType, uint16_t iApplyType, int& iApplyValue)	//@fixme532
{
	TVecApplys vec_Applys;
	if (!GetBasicApplys(iType, vec_Applys))
	{
...
bool DragonSoulTable::GetAdditionalApplyValue(uint8_t iType, uint16_t iApplyType, int& iApplyValue)	//@fixme532
{
	TVecApplys vec_Applys;
