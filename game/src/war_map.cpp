...
CWarMap::CWarMap(long lMapIndex, const TGuildWarInfo & r_info, TWarMapInfo * pkWarMapInfo, uint32_t dwGuildID1, uint32_t dwGuildID2)
{
...
void CWarMap::IncMember(LPCHARACTER ch)
{
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "[LS;490]");
	}
...
bool CWarMapManager::GetStartPosition(long lMapIndex, uint8_t bIdx, PIXEL_POSITION & pos)
{
	assert(bIdx < 3);
...
long CWarMapManager::CreateWarMap(const TGuildWarInfo& guildWarInfo, uint32_t dwGuildID1, uint32_t dwGuildID2)
{
