...
void CHARACTER::OpenHuntingWindowMain()
{
	if (GetQuestFlag("hunting_system.is_active") == 1 and GetLevel() >= GetQuestFlag("hunting_system.level")) 
	{
		uint32_t actLevel = GetQuestFlag("hunting_system.level");
		uint8_t actType = GetQuestFlag("hunting_system.type");
		uint32_t actCount = GetQuestFlag("hunting_system.count");
...
void CHARACTER::UpdateHuntingMission(uint32_t dwMonsterVnum)
{
	if (!GetDesc())
		return;
...
void CHARACTER::ReciveHuntingRewards()
{
	...
	{ 
		...
		uint32_t exp = (uint32_t)((exp_table[MINMAX(0, level, PLAYER_EXP_TABLE_MAX)] * proc) / 100);
...
{
	uint32_t MissionLevel = GetQuestFlag("hunting_system.level");
	
	uint32_t itemVnum, itemCount;
...
void CHARACTER::SendRandomItemPacket(bool IsSelectWindow)
{
	...
	if (MissionLevel >= 1 and MissionLevel <= 20) {
		uint32_t TableSize = (sizeof THuntingRandomItem_01_20/sizeof THuntingRandomItem_01_20[0]);
	...
	if (MissionLevel >= 21 and MissionLevel <= 40) {
		uint32_t TableSize = (sizeof THuntingRandomItem_21_40/sizeof THuntingRandomItem_21_40[0]);
		for (int i = 0; i < TableSize; ++i) {
			TPacketGCReciveRandomItems pack;
			pack.bHeader = HEADER_GC_HUNTING_RECIVE_RAND_ITEMS;
			pack.bWindow = IsSelectWindow;
	...
	if (MissionLevel >= 41 and MissionLevel <= 60) {
		uint32_t TableSize = (sizeof THuntingRandomItem_41_60/sizeof THuntingRandomItem_41_60[0]);
		for (int i = 0; i < TableSize; ++i) {
			TPacketGCReciveRandomItems pack;
			pack.bHeader = HEADER_GC_HUNTING_RECIVE_RAND_ITEMS;
			pack.bWindow = IsSelectWindow;
	...
	if (MissionLevel >= 61 and MissionLevel <= 80) {
		uint32_t TableSize = (sizeof THuntingRandomItem_61_80/sizeof THuntingRandomItem_61_80[0]);
		for (int i = 0; i < TableSize; ++i) {
			TPacketGCReciveRandomItems pack;
			pack.bHeader = HEADER_GC_HUNTING_RECIVE_RAND_ITEMS;
			pack.bWindow = IsSelectWindow;
	...
	if (MissionLevel >= 81 and MissionLevel <= 90) {
		uint32_t TableSize = (sizeof THuntingRandomItem_81_90/sizeof THuntingRandomItem_81_90[0]);
		for (int i = 0; i < TableSize; ++i) {
			TPacketGCReciveRandomItems pack;
			pack.bHeader = HEADER_GC_HUNTING_RECIVE_RAND_ITEMS;
			pack.bWindow = IsSelectWindow;
...
int CHARACTER::GetMinMaxMoney(uint32_t missionLevel, bool AskMax)
{
...
int CHARACTER::GetRandomMoney(uint32_t missionLevel)
{
	for (int i = 0; i < HUNTING_MONEY_TABLE_SIZE; ++i)
	{
		const SHuntingRewardMoney & info = THuntingRewardMoney[i];
...
int CHARACTER::GetMinMaxExp(uint32_t missionLevel, bool AskMax)
{
...
int CHARACTER::GetRandomExp(uint32_t missionLevel)
{
	int percent = 0;
	for (int i = 0; i < HUNTING_EXP_TABLE_SIZE; ++i)
	{
