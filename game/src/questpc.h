...
namespace quest
{
	...
	struct RewardData
	{
		...
		RewardData(RewardType t, uint32_t value1, int value2 = 0) :
			type(t),
			value1(value1),
			value2(value2)
		{
	...
	public:
		...
		void AutoGiveItem(uint32_t dwVnum, int count);
		void GiveExp(const string& label, uint32_t exp);

		void SetSendDoneFlag() { m_bShouldSendDone = true; }
		bool GetAndResetDoneFlag() { const bool temp = m_bShouldSendDone; m_bShouldSendDone = false; return temp; }
