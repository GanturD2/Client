...
static long FN_castle_map_index(int empire)
{
	...
	{
		case EMPIRE_RED:
			return MAP_EMPIREWAR01;

		case EMPIRE_YELLOW:
			return MAP_EMPIREWAR03;

		case EMPIRE_BLUE:
			return MAP_EMPIREWAR02;
	}
	return 0;
}
...
static bool FN_is_castle_map(int map_index)
{
	switch (map_index)
	{
		case MAP_EMPIREWAR01:
		case MAP_EMPIREWAR02:
		case MAP_EMPIREWAR03:
			return true;
	}
	return false;
}
...
LPCHARACTER castle_spawn_guard(int empire, uint32_t group_vnum, int region_index)
{
	LPCHARACTER	mob;
	int	sx, sy, ex, ey;
	long	map_index	= FN_castle_map_index(empire);
...
int castle_cost_of_hiring_guard(uint32_t group_vnum)
{
	switch (group_vnum)
	{
		case 9501:
...
bool castle_frog_to_empire_money(LPCHARACTER ch)
{
	...
	DO_ALL_FROG(i)
	{
		...
		GET_FROG(empire, i) = nullptr;
		npc->Dead(/*killer*/nullptr, /*immediate_dead*/true);
		return true;
	}
...
bool castle_is_my_castle(int empire, int map_index)
{
	...
	{
		case EMPIRE_RED:
			return (MAP_EMPIREWAR01 == map_index);

		case EMPIRE_YELLOW:
			return (MAP_EMPIREWAR03 == map_index);

		case EMPIRE_BLUE:
			return (MAP_EMPIREWAR02 == map_index);
	}
