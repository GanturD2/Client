...
void CLand::PutData(const TLand * data)
{
	...
	if (m_data.dwGuildID)
	{
		...
		if (r)
		{
			...
			auto snapshot = CHARACTER_MANAGER::Instance().GetCharactersByRaceNum(20040);
			if (!snapshot.empty())
			{
...
bool CLand::RequestCreateObject(uint32_t dwVnum, long lMapIndex, long x, long y, float xRot, float yRot, float zRot, bool checkAnother)
{
...
// BUILD_WALL
void CLand::DrawWall(uint32_t dwVnum, long nMapIndex, long& x, long& y, char length, float zRot)
{
	int rot = (int)zRot;
	rot = ((rot%360) / 90) * 90;
...
bool CLand::RequestCreateWallBlocks(uint32_t dwVnum, long nMapIndex, char wallSize, bool doorEast, bool doorWest, bool doorSouth, bool doorNorth)
{
	const TLand & r = GetData();
