...
namespace SnakeLair
{
	...
	EVENTFUNC(r_snakespawn_event)
	{
		...
		if (pEventInfo->bStep == 3)
		{
			if (pEventInfo->bSubStep == 1)
			{
				...
				SendNoticeMap(LC_TEXT("[1740]Destroy the Metin Stones in the correct order."), pMap->GetMapIndex(), true);
			}
		...
		if (pEventInfo->bStep == 4)
		{
			if (pEventInfo->bSubStep == 1)
			{
				...
				SendNoticeMap(LC_TEXT("[1741]Kill all the monsters."), pMap->GetMapIndex(), true);
		...
		if (pEventInfo->bStep == 5)
		{
			if (pEventInfo->bSubStep == 1)
			{
				...
				SendNoticeMap(LC_TEXT("[1742]Take the key from the monsters to unlock the Serpent Pillar."), pMap->GetMapIndex(), true);
		...
		if (pEventInfo->bStep == 6)
		{
			...
			if (pEventInfo->bSubStep == 2)
			{
				...
				SendNoticeMap(LC_TEXT("[1738]You have killed all the monsters and can now proceed to the boss level."), pMap->GetMapIndex(), true);
			}
			if (pEventInfo->bSubStep == 3)
			{
	...
	void CSnkMap::StartDungeon(LPCHARACTER pkChar)
	{
		...
		char szNotice[512];
		snprintf(szNotice, sizeof(szNotice), LC_TEXT("[1729]This is now available for %d minutes."), uint32_t(TIME_LIMIT_DUNGEON) / 60);
	...
	void CSnkMap::SetDungeonStep(uint8_t bStep)
	{
		...
		if (bStep == 2)
		{
			SendNoticeMap(LC_TEXT("[1741]Kill all the monsters."), GetMapIndex(), true);
		...
		{
			char szNotice[512];
			snprintf(szNotice, sizeof(szNotice), LC_TEXT("[1732]You have killed all the monsters. You will be transported to level %d. %d seconds remaining until then."), 3, 5);
		...
		{
			char szNotice[512];
			snprintf(szNotice, sizeof(szNotice), LC_TEXT("[1733]You have destroyed all of the Metins. You will be transported to level %d. %d seconds remaining until then."), 4, 5);
			SendNoticeMap(szNotice, GetMapIndex(), true);
		...
		{
			char szNotice[512];
			snprintf(szNotice, sizeof(szNotice), LC_TEXT("[1733]You have destroyed all of the Metins. You will be transported to level %d. %d seconds remaining until then."), 5, 5);
			SendNoticeMap(szNotice, GetMapIndex(), true);
		...
		{
			char szNotice[512];
			snprintf(szNotice, sizeof(szNotice), LC_TEXT("[1733]You have destroyed all of the Metins. You will be transported to level %d. %d seconds remaining until then."), 6, 5);
			SendNoticeMap(szNotice, GetMapIndex(), true);
		...
		if (bStep == 7)
		{
			...
			char szNotice[512];
			snprintf(szNotice, sizeof(szNotice), LC_TEXT("[1733]You have destroyed all of the Metins. You will be transported to level %d. %d seconds remaining until then."), 7, 5);
			SendNoticeMap(szNotice, GetMapIndex(), true);
	...
	void CSnkMap::OnKill(LPCHARACTER pkMonster, LPCHARACTER pKiller)
	{
		...
		if ((bStep == 3) && (pkMonster->GetRaceNum() == SNAKEMETIN_FIRE || pkMonster->GetRaceNum() == SNAKEMETIN_ICE || pkMonster->GetRaceNum() == SNAKEMETIN_WIND || pkMonster->GetRaceNum() == SNAKEMETIN_EARTH))
		{
			if (static_cast<uint32_t>(stoneKillSort[GetElementSort()][GetKillCountMonsters()]) == pkMonster->GetRaceNum())
			{
				...
				{
					char szNotice[512];
					snprintf(szNotice, sizeof(szNotice), LC_TEXT("[1727]You have destroyed Metin %d. Now destroy Metin %d."), GetKillCountMonsters(), (GetKillCountMonsters() + 1)); // Need to make the maths
					SendNoticeMap(szNotice, GetMapIndex(), true);
				}
			}
			else
			{
				SendNoticeMap(LC_TEXT("[1725]The destroyed Metin isn't reacting. All the Metins have appeared again."), GetMapIndex(), true);
		...
		if (bStep == 6)
		{
			if (pkMonster->GetRaceNum() == 6750 || pkMonster->GetRaceNum() == 6753 || pkMonster->GetRaceNum() == 6754)
			{
				if (!pKiller)
					return;
		...
		if (bStep == 7)
		{
			...
			{
				char szNotice[512];
				snprintf(szNotice, sizeof(szNotice), LC_TEXT("[1739]You have completed the Serpent Temple. You will be transported out of the dungeon in %d seconds."), 20);
				SendNoticeMap(szNotice, GetMapIndex(), true);
	...
	void CSnkMap::OnKillBlackSmith(LPITEM pkItem, LPCHARACTER pkChar, LPCHARACTER pkBlackSmith)
	{
		...
		{
			SendNoticeMap(LC_TEXT("[1747]The prison has been unlocked. The blacksmith smiles at you."), GetMapIndex(), true);
		}
		...
		if (GetKillCountPilars() >= 4)
		{
			...
			SendNoticeMap(LC_TEXT("[1736]The special Serpent Temple Smith appears!"), GetMapIndex(), true);
	...
	void CSnkMap::OnKillPilar(LPITEM pkItem, LPCHARACTER pkChar, LPCHARACTER pkPilar)
	{
		...
		if (GetSnakePilar2())
		{
			if (GetSnakePilar2()->GetVID() == pkPilar->GetVID())
			{
				...
				{
					char szNotice[512];
					snprintf(szNotice, sizeof(szNotice), LC_TEXT("[1746]You have to unlock the other pillar first."));
					SendNoticeMap(szNotice, GetMapIndex(), true);
					return;
				}
				GetSnakePilar2()->Dead();
		...
		{
			char szNotice[512];
			snprintf(szNotice, sizeof(szNotice), LC_TEXT("[1745]Successfully opened!"));
			SendNoticeMap(szNotice, GetMapIndex(), true);
		}
...
#ifdef ENABLE_YOHARA_SYSTEM
	int CSnk::GetSungmaQueenDungeonValue(LPCHARACTER pkChar, int iPoint)
	{
		if (!pkChar)
			return 0;
