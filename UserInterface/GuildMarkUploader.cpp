...
bool CGuildMarkUploader::__Save(const char * c_szFileName) const
{
	return true;
}
...
bool CGuildMarkUploader::Connect(const CNetworkAddress & c_rkNetAddr, uint32_t dwHandle, uint32_t dwRandomKey, uint32_t dwGuildID,
...
bool CGuildMarkUploader::ConnectToSendSymbol(const CNetworkAddress & c_rkNetAddr, uint32_t dwHandle, uint32_t dwRandomKey,
...
bool CGuildMarkUploader::__LoginState_Process()
{
	...
	if (!__AnalyzePacket(HEADER_GC_KEY_AGREEMENT_COMPLETED, sizeof(TPacketKeyAgreementCompleted),
						 &CGuildMarkUploader::__LoginState_RecvKeyAgreementCompleted))
		return false;
#endif
...
bool CGuildMarkUploader::__SendSymbolPacket()
{
	...
	kPacketSymbolUpload.size = msl::truncate_cast<uint8_t>(sizeof(TPacketCGSymbolUpload) + m_pbySymbolBuf.size());
...
#ifdef _DEBUG
	printf("__SendSymbolPacket : [GuildID:%d/PacketSize:%d/CRC:%d]\n", m_dwGuildID, kPacketSymbolUpload.size, m_dwSymbolCRC32);
#endif
