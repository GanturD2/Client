...
void CPythonTicketLogs::AddLogDetails(TSubPacketTicketLogsData p)
{
	m_vecData_A.clear();	

	for(int i = 0; i < TICKET_MAX_LOGS_GENERAL; i++)
	{
		TSubPacketTicketLogs mex = p.logs[i];
		m_vecData_A.push_back(mex);
	}
}
void CPythonTicketLogsReply::AddLogDetails(TSubPacketTicketLogsDataReply p)
{
	m_vecData_B.clear();

	for(int i = 0; i < TICKET_MAX_LOGS_FROM_REPLY; i++)
	{
		TSubPacketTicketLogsReply mex = p.logs[i];
		m_vecData_B.push_back(mex);
	}
}

TSubPacketTicketLogs CPythonTicketLogs::Request(int id)
{
	if(m_vecData_A.size() < id)
	{
		TSubPacketTicketLogs log;
...
TSubPacketTicketLogsReply CPythonTicketLogsReply::Request(int id)
{
	if(m_vecData_B.size() < id)
	{
		TSubPacketTicketLogsReply log;
...
PyObject * ticketLoadLogs(PyObject* poSelf, PyObject* poArgs)
{
	...
	TSubPacketTicketLogs p = CPythonTicketLogs::instance()->Request(id);
...
PyObject * ticketLoadLogsReply(PyObject* poSelf, PyObject* poArgs)
{
	...
	TSubPacketTicketLogsReply p = CPythonTicketLogsReply::instance()->Request(id);
