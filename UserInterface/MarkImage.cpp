...
bool CGuildMarkImage::SaveBlockFromCompressedData(uint32_t posBlock, const uint8_t * pbComp, uint32_t dwCompSize)
{
	...
	if (LZO_E_OK !=
		lzo1x_decompress_safe(pbComp, dwCompSize, reinterpret_cast<uint8_t *>(apxBuf), reinterpret_cast<lzo_uint *>(&sizeBuf),
...
void SGuildMarkBlock::Compress(const Pixel * pxBuf)
{
	...
	{
		sys_err("SGuildMarkBlock::Compress: Error! %u > %u", sizeof(Pixel) * SIZE, m_sizeCompBuf);
		return;
	}
