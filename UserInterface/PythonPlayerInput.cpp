...
bool CPythonPlayer::__IsTarget() const
{
	return 0 != __GetTargetVID();
}

bool CPythonPlayer::__IsSameTargetVID(uint32_t dwVID) const
{
...
uint32_t CPythonPlayer::GetTargetVID() const
{
	return __GetTargetVID();
}


CInstanceBase * CPythonPlayer::__GetTargetActorPtr() const
{
	...
	if (pkInstTarget)
	{
		if (pkInstMain->IsTargetableInstance(*pkInstTarget))
		{
			...
			pkInstMain->GetGraphicThingInstanceRef().SetFlyEventHandler(
				CPythonPlayerEventHandler::GetSingleton().GetNormalBowAttackFlyEventHandler(pkInstMain, pkInstTarget));
...
CInstanceBase * CPythonPlayer::__GetSkillTargetInstancePtr(const CPythonSkill::TSkillData & rkSkillData) const
{
...
CInstanceBase * CPythonPlayer::__GetDeadTargetInstancePtr() const
{
	if (!__IsTarget())
		return nullptr;
...
CInstanceBase * CPythonPlayer::__GetAliveTargetInstancePtr() const
{
	if (!__IsTarget())
		return nullptr;
...
void CPythonPlayer::OpenCharacterMenu(uint32_t dwVictimActorID) const
{
	CInstanceBase * pkInstMain = Instance().NEW_GetMainActorPtr();
	if (!pkInstMain)
		return;
	...
	if (pkInstMain->IsBattleFieldMap() || pkInstTarget->IsBattleFieldMap())
		return;
#endif
	...
	if (pkInstTarget->IsShop()) {
		PyCallClassMemberFunc(m_ppyGameWindow, "SetPCTargetBoard", Py_BuildValue("(is)", 0, pkInstTarget->GetNameString()));
		return;
	}
#endif
	...
	PyCallClassMemberFunc(m_ppyGameWindow, "SetPCTargetBoard",
						  Py_BuildValue("(is)", pkInstTarget->GetVirtualID(), pkInstTarget->GetNameString()));
}

void CPythonPlayer::__OnClickItem(CInstanceBase & rkInstMain, uint32_t dwItemID) const {}
...
void CPythonPlayer::__OnPressActor(CInstanceBase & rkInstMain, uint32_t dwPickedActorID, bool isAuto)
{
	...
	rkInstMain.NEW_AttackToDestInstanceDirection(rkInstVictim,
												 rkPlayerEventHandler.GetNormalBowAttackFlyEventHandler(&rkInstMain, &rkInstVictim));
}
...
bool CPythonPlayer::__IsMovableGroundDistance(CInstanceBase & rkInstMain, const TPixelPosition & c_rkPPosPickedGround) const
{
...
void CPythonPlayer::NEW_ResetCameraRotation() const
{
	CCamera * pkCmrCur = CCameraManager::Instance().GetCurrentCamera();
...
bool CPythonPlayer::__CanAttack()
{
	...
	{
		TraceError("__CanAttack GetSkillGrade(109) %d", GetSkillGrade(109));
		return false;
	}
#endif
...
void CPythonPlayer::NEW_GetMultiKeyDirRotation(bool isLeft, bool isRight, bool isUp, bool isDown, float * pfDirRot) const
{
	float fScrX = 0.0f;
	float fScrY = 0.0f;
...
void CPythonPlayer::NEW_GetMouseDirRotation(float fScrX, float fScrY, float * pfDirRot) const
{
...
float CPythonPlayer::GetDegreeFromPosition(int ix, int iy, int iHalfWidth, int iHalfHeight) const
{
...
bool CPythonPlayer::__IsReservedUseSkill(uint32_t dwSkillSlotIndex) const
{
	if (MODE_USE_SKILL != m_eReservedMode)
		return false;
