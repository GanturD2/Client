...
void CPythonPlayer::ClearAffects() const
{
...
bool CPythonPlayer::__CheckShortArrow(const TSkillInstance & rkSkillInst, const CPythonSkill::TSkillData & rkSkillData)
{
...
bool CPythonPlayer::__CheckShortMana(const TSkillInstance & rkSkillInst, const CPythonSkill::TSkillData & rkSkillData)
{
...
bool CPythonPlayer::__CheckShortLife(const TSkillInstance & rkSkillInst, const CPythonSkill::TSkillData & rkSkillData)
{
...
bool CPythonPlayer::__CheckRestSkillCoolTime(uint32_t dwSlotIndex) const
{
	if (!m_sysIsCoolTime)
		return false;
...
bool CPythonPlayer::__CheckDashAffect(const CInstanceBase & rkInstMain) const
{
...
uint32_t CPythonPlayer::__GetSkillTargetRange(const CPythonSkill::TSkillData & rkSkillData)
{
...
bool CPythonPlayer::__CanUseSkill()
{
	...
	if (pkInstMain->IsMountingHorse() && pkInstMain->IsNewMount() && (GetSkillGrade(107) < 1 && GetSkillLevel(107) < 20))
	{
	...
	{
		TraceError("__CanUseSkill GetSkillGrade(109) %d", GetSkillGrade(109));
		return false;
	}
#endif
		...
		if (pkInstTarget)
		{
			if (pSkillData->IsOnlyForAlliance())
			{
				if (pkInstMain == pkInstTarget)
				{
					...
					{
						PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill",
											  Py_BuildValue("(is)", GetMainCharacterIndex(), "CANNOT_USE_SELF"));
						return false;
					}
				}
				...
				else
				{
					...
					{
						PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill",
											  Py_BuildValue("(is)", GetMainCharacterIndex(), "ONLY_FOR_ALLIANCE"));
						return false;
					}
				}
			}
			else
			{
				...
				{
					PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill",
										  Py_BuildValue("(is)", GetMainCharacterIndex(), "CANNOT_ATTACK_ENEMY_IN_SAFE_AREA"));
					return false;
				}
...
{
	CItemData * pItemData;
	if (CItemManager::Instance().GetItemDataPointer(GetItemIndex(TItemPos(EQUIPMENT, c_Equipment_Arrow)), &pItemData))
	{
...
extern const uint32_t c_iSkillIndex_AutoAttack;
#endif
...
#ifdef ENABLE_PASSIVE_ATTR
	// AutoAttack
	if (c_iSkillIndex_AutoAttack == dwSkillIndex)
	{
		uint32_t dwSlotIndex;
		if (!GetSkillSlotIndex(dwSkillIndex, &dwSlotIndex))
			return false;
		...
		if (GetTargetVID())
			__SetAutoAttackTargetActorID(GetTargetVID());
		else
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NEED_TARGET"));
