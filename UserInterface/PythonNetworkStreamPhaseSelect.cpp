...
void CPythonNetworkStream::SelectPhase()
{
	...
	switch (header)
	{
		...
		case HEADER_GC_LOGIN_SUCCESS_NEWSLOT:
...
bool CPythonNetworkStream::SendCreateCharacterPacket(uint8_t index, const char * name, uint8_t job, uint8_t shape, uint8_t byCON,
													 uint8_t byINT, uint8_t bySTR, uint8_t byDEX)
{
...
bool CPythonNetworkStream::__RecvPlayerDestroySuccessPacket()
{
	...
	m_akSimplePlayerInfo[packet.account_index] = {};
