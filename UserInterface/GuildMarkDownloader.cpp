...
bool CGuildMarkDownloader::ConnectToRecvSymbol(const CNetworkAddress & c_rkNetAddr, uint32_t dwHandle, uint32_t dwRandomKey,
...
bool CGuildMarkDownloader::__LoginState_RecvMarkBlock()
{
	...
	for (uint32_t i = 0; i < kPacket.count; ++i)
	{
		...
		else
		{
			...
			CGuildMarkManager::Instance().SaveBlockFromCompressedData(kPacket.imgIdx, posBlock, reinterpret_cast<const uint8_t *>(compBuf),
																	  compSize);
		}
	}
	...
	if (kPacket.count > 0)
	{
		...
		if (CGuildMarkManager::Instance().GetMarkImageFilename(kPacket.imgIdx, imagePath))
		{
			...
			{
				auto * pkGrpImg = msl::inherit_cast<CGraphicImage *>(pResource);
				pkGrpImg->Reload();
			}
		}
	}
