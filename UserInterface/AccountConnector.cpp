...
bool CAccountConnector::__AuthState_RecvAuthSuccess()
{
	...
	{
		uint32_t dwPanamaKey =
			kAuthSuccessPacket.dwLoginKey ^ g_adwEncryptKey[0] ^ g_adwEncryptKey[1] ^ g_adwEncryptKey[2] ^ g_adwEncryptKey[3];
...
bool CAccountConnector::__AnalyzeVarSizePacket(uint32_t uHeader, bool (CAccountConnector::*pfnDispatchPacket)(int))
{
	uint8_t bHeader;
	if (!Peek(sizeof(bHeader), &bHeader))
		return true;
