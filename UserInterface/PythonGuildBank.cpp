...
void CPythonGuildBank::OpenGuildBank(int iSize)
{
	...
	for (uint32_t i = 0; i < m_GuildBankItemInstanceVector.size(); ++i)
	{
...
void CPythonGuildBank::SetGuildBankItemData(uint32_t dwSlotIndex, const TItemData& rItemData)
{
...
void CPythonGuildBank::DelGuildBankItemData(uint32_t dwSlotIndex)
{
	if (dwSlotIndex >= m_GuildBankItemInstanceVector.size())
	{
...
BOOL CPythonGuildBank::GetGuildBankItemDataPtr(uint32_t dwSlotIndex, TItemData** ppInstance)
{
	if (dwSlotIndex >= m_GuildBankItemInstanceVector.size())
	{
...
BOOL CPythonGuildBank::GetSlotGuildBankItemID(uint32_t dwSlotIndex, uint32_t* pdwItemID)
{
	if (dwSlotIndex >= m_GuildBankItemInstanceVector.size())
	{
		TraceError("CPythonGuildBank::GetGuildBankSlotItemID(dwSlotIndex=%d) - Strange slot index", dwSlotIndex);
...
uint32_t CPythonGuildBank::GetGuildBankSize()
{
