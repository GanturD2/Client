...
uint32_t CInstanceBase::NEW_GetDistanceFromMainInstance()
{
	if (m_GraphicThingInstance.IsMainInstance())
		return 0;

	TPixelPosition kPPosInst;
	NEW_GetPixelPosition(&kPPosInst);
	...
	kPPosMain -= kPPosInst;
	return kPPosMain.x * kPPosMain.x + kPPosMain.y * kPPosMain.y;
}
#endif
...
float CInstanceBase::NEW_GetDistanceFromDestPixelPosition(const TPixelPosition & c_rkPPosDst)
{
	...
	TPixelPosition kPPosDir = c_rkPPosDst - kPPosCur;
...
BOOL CInstanceBase::CheckAdvancing()
{
	...
	if (!__IsMainInstance() && !IsAttacking())
	{
		...
		{
			CPythonCharacterManager & rkChrMgr = CPythonCharacterManager::Instance();
			for (CPythonCharacterManager::CharacterIterator i = rkChrMgr.CharacterInstanceBegin(); i != rkChrMgr.CharacterInstanceEnd();
				 ++i)
			{
				CInstanceBase * pkInstEach = *i;
				if (pkInstEach == this)
	...
	auto iStep = int(D3DXVec3Length(&rv3MoveDirection) / 10.0f);
...
bool CInstanceBase::IsAffect(uint32_t uAffect) const
{
...
void CInstanceBase::ProcessHitting(uint32_t dwMotionKey, CInstanceBase * pVictimInstance) const
{
	assert(!"-_-" && "CInstanceBase::ProcessHitting");
}

void CInstanceBase::ProcessHitting(uint32_t dwMotionKey, uint8_t byEventIndex, CInstanceBase * pVictimInstance) const
{
	assert(!"-_-" && "CInstanceBase::ProcessHitting");
}
...
void CInstanceBase::SetAutoAffect(bool val)
{
	m_GraphicThingInstance.SetAutoAffect(val);
}
#endif
