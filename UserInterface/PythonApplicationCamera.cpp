...
void CPythonApplication::__UpdateCamera()
{
	...
	if (CAMERA_MODE_BLEND == m_iCameraMode)
	{
		...
		float fxCenter = BlendValueByLinear(fElapsedTime, m_fBlendCameraBlendTime, m_kEventCameraSetting.v3CenterPosition.x,
											m_kEndBlendCameraSetting.v3CenterPosition.x);
		float fyCenter = BlendValueByLinear(fElapsedTime, m_fBlendCameraBlendTime, m_kEventCameraSetting.v3CenterPosition.y,
											m_kEndBlendCameraSetting.v3CenterPosition.y);
		float fzCenter = BlendValueByLinear(fElapsedTime, m_fBlendCameraBlendTime, m_kEventCameraSetting.v3CenterPosition.z,
											m_kEndBlendCameraSetting.v3CenterPosition.z);

		float fDistance =
			BlendValueByLinear(fElapsedTime, m_fBlendCameraBlendTime, m_kEventCameraSetting.fZoom, m_kEndBlendCameraSetting.fZoom);
		float fPitch =
			BlendValueByLinear(fElapsedTime, m_fBlendCameraBlendTime, m_kEventCameraSetting.fPitch, m_kEndBlendCameraSetting.fPitch);
		float fRotation =
			BlendValueByLinear(fElapsedTime, m_fBlendCameraBlendTime, m_kEventCameraSetting.fRotation, m_kEndBlendCameraSetting.fRotation);
		...
		float fViewDir = BlendValueByLinear(fElapsedTime, m_fBlendCameraBlendTime, m_kEventCameraSetting.kCmrPos.m_fViewDir,
											m_kEndBlendCameraSetting.kCmrPos.m_fViewDir);
		float fCrossDir = BlendValueByLinear(fElapsedTime, m_fBlendCameraBlendTime, m_kEventCameraSetting.kCmrPos.m_fCrossDir,
	...
	else if (CAMERA_MODE_STAND == m_iCameraMode)
	{
		...
		m_pyGraphic.SetPositionCamera(m_kEventCameraSetting.v3CenterPosition.x, m_kEventCameraSetting.v3CenterPosition.y,
									  m_kEventCameraSetting.v3CenterPosition.z + pMainCamera->GetTargetHeight(), fDistance, fPitch,
									  fRotation);
	}
	else if (CAMERA_MODE_NORMAL == m_iCameraMode)
	{
		float fDistance, fPitch, fRotation, fHeight;
		GetCamera(&fDistance, &fPitch, &fRotation, &fHeight);
		m_pyGraphic.SetPositionCamera(m_v3CenterPosition.x, m_v3CenterPosition.y, m_v3CenterPosition.z + pMainCamera->GetTargetHeight(),
	...
	m_SoundManager.SetPosition(m_v3CenterPosition.x, m_v3CenterPosition.y, m_v3CenterPosition.z); // Listener - ĳ���� ��ġ
	m_SoundManager.SetDirection(c_rv3CameraDirection.x, c_rv3CameraDirection.y, c_rv3CameraDirection.z, c_rv3CameraUp.x, c_rv3CameraUp.y,
								c_rv3CameraUp.z);
...
void CPythonApplication::GetCamera(float * Distance, float * Pitch, float * Rotation, float * DestinationHeight) const
{
	CCamera * pCurrentCamera = CCameraManager::Instance().GetCurrentCamera();
...
float CPythonApplication::GetRotation() const
{
	return CCameraManager::Instance().GetCurrentCamera()->GetRoll();
}

float CPythonApplication::GetPitch() const
{
	return CCameraManager::Instance().GetCurrentCamera()->GetPitch();
}

bool CPythonApplication::IsLockCurrentCamera() const
{
...
void CPythonApplication::CpyDefaultCamera(const SCameraSetting& c_rCameraSetting)
{
	m_iCameraMode = CAMERA_MODE_NORMAL;
	m_fBlendCameraStartTime = 0.0f;
	m_fBlendCameraBlendTime = 0.0f;
	...
	{
		CCameraManager::Instance().SetCurrentCamera(CCameraManager::DEFAULT_PERSPECTIVE_CAMERA);
		memcpy(&m_DefaultCameraSetting, &c_rCameraSetting, sizeof(SCameraSetting));
		SetCameraSetting(m_DefaultCameraSetting);
	}
}

bool CPythonApplication::GetDefaultCameraSetting(SCameraSetting* pCameraSetting) const
{
...
void CPythonApplication::SetCameraSetting(const SCameraSetting & c_rCameraSetting)
{
	...
	m_pyGraphic.SetPositionCamera(c_rCameraSetting.v3CenterPosition.x, c_rCameraSetting.v3CenterPosition.y,
								  c_rCameraSetting.v3CenterPosition.z, c_rCameraSetting.fZoom, c_rCameraSetting.fPitch,
...
void CPythonApplication::GetCameraSetting(SCameraSetting * pCameraSetting) const
{
	pCameraSetting->v3CenterPosition = m_v3CenterPosition;
	pCameraSetting->kCmrPos = m_kCmrPos;
...
void CPythonApplication::SaveCameraSetting(const char * c_szFileName) const
{
	...
	PrintfTabs(fPtr.get(), 0, "CenterPos %f %f %f\n", CameraSetting.v3CenterPosition.x, CameraSetting.v3CenterPosition.y,
			   CameraSetting.v3CenterPosition.z);
	PrintfTabs(fPtr.get(), 0, "CameraSetting %f %f %f\n", CameraSetting.fZoom, CameraSetting.fPitch, CameraSetting.fRotation);
	PrintfTabs(fPtr.get(), 0, "CmrPos %f %f %f\n", CameraSetting.kCmrPos.m_fUpDir, CameraSetting.kCmrPos.m_fViewDir,
			   CameraSetting.kCmrPos.m_fCrossDir);
	PrintfTabs(fPtr.get(), 0, "Line \"x;%d|y;%d|z;%d|distance;%d|pitch;%d|rot;%d|up;%d|view;%d|cross;%d\"\n",
			   int(CameraSetting.v3CenterPosition.x), int(CameraSetting.v3CenterPosition.y), int(CameraSetting.v3CenterPosition.z),
			   int(CameraSetting.fZoom), int(CameraSetting.fPitch), int(CameraSetting.fRotation), int(CameraSetting.kCmrPos.m_fUpDir),
			   int(CameraSetting.kCmrPos.m_fViewDir), int(CameraSetting.kCmrPos.m_fCrossDir));
}
