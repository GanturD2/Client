...
const long c_lNormal_Waiting_Time = QUEST_WAIT_TIME;	//@custom010	//(Original: 10)
const int c_fLine_Temp = 16;

void ShowArgument(script::TArgList & rArgumentList)
{
	for (auto & itor : rArgumentList)
	{
...
int CPythonEventManager::RegisterEventSetFromString(const std::string & strScript)
{
	...
	// SCRIPT_PARSING_FAILURE_CLEAR_BUG __ClearEventSetp
	pEventSet->pCurrentTextLine = nullptr;
...
void CPythonEventManager::UpdateEventSet(int iIndex, int ix, int iy)
{
	...
	pEventSet->lLastDelayTime = std::max<int>(0, pEventSet->lLastDelayTime - lElapsedTime);
	...
	switch (iEventType)
	{
		...
		case EVENT_TYPE_QUEST_BUTTON:
		{
			...
			else
			{
				...
				{
					PyCallClassMemberFunc(m_poInterface, "BINARY_RecvQuest", Py_BuildValue("(isss)", idx, name, c_rstType.c_str(), c_rstFile.c_str()));
				}
			}
			break;
		}
		...
		case EVENT_TYPE_IMAGE_SHOWTOOLTIP_BY_CELL:
		{
#ifdef ENABLE_REVERSED_FUNCTIONS_EX
			TItemPos Cell;
			Cell.window_type = static_cast<uint8_t>(atoi(GetArgument("window", ScriptCommand.argList)));
			Cell.window_type = static_cast<uint16_t>(atoi(GetArgument("pos", ScriptCommand.argList)));
			if (Cell.IsValidCell())
			{
				PyCallClassMemberFunc(pEventSet->poEventHandler, "OnInsertImageShowItemToolTipByCell", Py_BuildValue("(ii)", Cell.window_type, Cell.window_type));
				pEventSet->iAdjustLine += 2;
			}
			break;
#endif

			int window_type = atoi(GetArgument("window_type", ScriptCommand.argList));
			int cell = atoi(GetArgument("cell", ScriptCommand.argList));

			PyCallClassMemberFunc(pEventSet->poEventHandler, "OnInsertImageShowItemToolTipByCell", Py_BuildValue("(ii)", window_type, cell));
		...
		{
			PyCallClassMemberFunc(m_poInterface, "BINARY_OpenSelectItemWindowEx", Py_BuildValue("()"));
			break;
		}
#endif

#ifdef ENABLE_REVERSED_FUNCTIONS_EX
		case EVENT_TYPE_CLOSE_CINEMA:
		{
			PyCallClassMemberFunc(pEventSet->poEventHandler, "CloseSelf", Py_BuildValue("()"));
			PyCallClassMemberFunc(m_poInterface, "ShowAllQuestDialog", Py_BuildValue("()"));
			break;
		}

		case EVENT_TYPE_SET_KEYBOARD_MOUSE_LOCK:
		{
			const int enable = atoi(GetArgument("value", ScriptCommand.argList));

			if (enable)
			{
				gs_bEnableMouseClickEvents = false;
				gs_bEnableMouseWheelEvents = false;
			}
			else
			{
				gs_bEnableMouseClickEvents = true;
				gs_bEnableMouseWheelEvents = true;
			}
			break;
		}

		case EVENT_TYPE_RESTORE_CAMERA_EXCEPT_POSITION:
		{
			IAbstractApplication::SCameraSetting curCameraSetting;
			if (CPythonApplication::Instance().GetDefaultCameraSetting(&curCameraSetting))
			{
				IAbstractApplication::SCameraSetting newCameraSetting;
				GetCameraSettingFromArgList(ScriptCommand.argList, &newCameraSetting);
				newCameraSetting.v3CenterPosition = curCameraSetting.v3CenterPosition;
				rApp.CpyDefaultCamera(newCameraSetting);
			}
			break;
		}
#endif
...
void CPythonEventManager::RenderEventSet(int iIndex)
{
	...
	int iCount = 0;
	for (TScriptTextLineList::iterator itor = pEventSet->ScriptTextLineList.begin(); itor != pEventSet->ScriptTextLineList.end(); ++itor, ++iCount)	//@fixme441
	{
		...
		TTextLine& rkLine = *itor;	//@fixme441
...
void CPythonEventManager::MakeNextButton(TEventSet * pEventSet, int iButtonType) const
{
...
void CPythonEventManager::MakeQuestion(TEventSet * pEventSet, script::TArgList & rArgumentList) const
{
...
void CPythonEventManager::SelectAnswer(int iIndex, int iAnswer) const
{
	if (!CheckEventSetIndex(iIndex))
		return;
...
void CPythonEventManager::RefreshLinePosition(TEventSet * pEventSet) const
{
	//int iCount = 0;
	for (auto & rkLine : pEventSet->ScriptTextLineList)
	{
...
void CPythonEventManager::__AddSpace(TEventSet & rEventSet, int iSpace) const
{
...
bool CPythonEventManager::CheckEventSetIndex(int iIndex) const
{
	if (iIndex < 0)
		return false;
...
CPythonEventManager::CPythonEventManager() : m_strLeftTimeString("³²Àº ½Ã°£ : %dÃÊ")
{
	...
	EventTypeMap["INSERT_IMAGE_SHOWTOOLTIP_BY_CELL"] = EVENT_TYPE_IMAGE_SHOWTOOLTIP_BY_CELL;
#if defined(ENABLE_GEM_SYSTEM) && defined(ENABLE_GEM_SHOP)
	EventTypeMap["SELECT_ITEM_EX"] = EVENT_TYPE_SELECT_ITEM_EX;
#endif
}

CPythonEventManager::~CPythonEventManager() = default;
