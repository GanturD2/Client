...
public:
	...
	bool LoadFromMemory(const uint8_t * c_pbMap);
	bool LoadHeaderFromMemory(const uint8_t * c_pbMap);
	bool Copy(int miplevel, uint8_t * pbDest, long lDestPitch) const;

	void Decompress(int miplevel, uint32_t * pdwDest);
	void DecompressDXT1(int miplevel, uint32_t * pdwDest);
	void DecompressDXT3(int miplevel, uint32_t * pdwDest);
	void DecompressDXT5(int level, uint32_t * pdwDest);
	void DecompressARGB(int level, uint32_t * pdwDest);

	void DecodePixelFormat(CHAR * strPixelFormat, XDDPIXELFORMAT * pxddpf);

	void Unextract(uint8_t * pbDest, int iWidth, int iHeight, int iPitch);
};
