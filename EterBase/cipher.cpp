...
struct BlockCipherAlgorithm
{
	...
	virtual SymmetricCipher * CreateEncoder(const CryptoPP::byte * key, size_t keylen, const CryptoPP::byte * iv) const = 0;
	virtual SymmetricCipher * CreateDecoder(const CryptoPP::byte * key, size_t keylen, const CryptoPP::byte * iv) const = 0;
};

// Block cipher (with CTR mode) algorithm selector template class.
template <class T> struct BlockCipherDetail : BlockCipherAlgorithm
{
	BlockCipherDetail() = default;
	~BlockCipherDetail() override = default;

	int GetBlockSize() const override { return T::BLOCKSIZE; }

	int GetDefaultKeyLength() const override { return T::DEFAULT_KEYLENGTH; }

	int GetIVLength() const override { return T::IV_LENGTH; }

	SymmetricCipher * CreateEncoder(const CryptoPP::byte * key, size_t keylen, const CryptoPP::byte * iv) const override
	{
		return new typename CTR_Mode<T>::Encryption(key, keylen, iv);
	}

	SymmetricCipher * CreateDecoder(const CryptoPP::byte * key, size_t keylen, const CryptoPP::byte * iv) const override
	{
...
DH2KeyAgreement::DH2KeyAgreement() : dh2_(dh_) {}

DH2KeyAgreement::~DH2KeyAgreement() = default;
...
size_t DH2KeyAgreement::Prepare(void * buffer, size_t * length)
{
	...
	*length = data_length;
	auto * buf = static_cast<CryptoPP::byte *>(buffer);
...
bool DH2KeyAgreement::Agree(size_t agreed_length, const void * buffer, size_t length)
{
	...
	const auto * buf = static_cast<const CryptoPP::byte *>(buffer);
	return dh2_.Agree(shared_, spriv_key_, epriv_key_, buf, buf + spub_key_length);
}
