...
public:
	...
	void Encrypt(void * buffer, size_t length) const
	{
		assert(activated_);
		if (!activated_)
			return;
		encoder_->ProcessData(static_cast<CryptoPP::byte *>(buffer), static_cast<const CryptoPP::byte *>(buffer), length);
	}
	// Decrypts the given block of data. (no padding required)
	void Decrypt(void * buffer, size_t length) const
	{
		assert(activated_);
		if (!activated_)
			return;
		decoder_->ProcessData(static_cast<CryptoPP::byte *>(buffer), static_cast<const CryptoPP::byte *>(buffer), length);
	}
