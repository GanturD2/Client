...
{
	return (code >= 0x20 && code <= 0x2f) || (code >= 0x3a && code <= 0x40) || (code >= 0x5b && code <= 0x60) ||
		(code >= 0x7b && code <= 0x7e);
}
	...
	for (size_t srcIndex = 0; srcIndex < srcLen; ++srcIndex)
	{
		...
		if (Arabic_IsInMap(cur))
		{
			...
			wchar_t prev = 0;
			{
				...
				if ((srcIndex == 0) || (!Arabic_IsInMap(prev)) || (!Arabic_GetMap(prev, INITIAL) && !Arabic_GetMap(prev, MEDIAL)))
				{
			...
			wchar_t next = 0;
			{
				...
				if ((nextIndex == srcLen) || (!Arabic_IsInMap(next)) ||
