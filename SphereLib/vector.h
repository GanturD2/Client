...
public:
	...
	Vector3d operator/(const float s) const // DIVIDE BY SCALAR (/)
	{
		...
		const Vector3d Scaled(x * r, y * r, z * r);
		return (Scaled);
	};
	...
	{
		const float l = Length(); // get length.
		if (l != 0)
		{
			x /= l;
			y /= l;
...
{
	const Vector3d Scaled(v.x * s, v.y * s, v.z * s);
	return (Scaled);
};
