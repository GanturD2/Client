...
public:
	...
	void ComputeBindingDistance(const SpherePack * parent)
	{
	...
	void VisibilityTest(const Frustum & f, SpherePackCallback * callback, ViewState state);
	...
	void RangeTest(const Vector3d & p, float distance, SpherePackCallback * callback, ViewState state);

	void PointTest2d(const Vector3d & p, SpherePackCallback * callback, ViewState state);
...
public:
	SpherePackFifo(int fifosize)
	{
		...
		mSP = 0;
		mBottom = 0;
		mFifo.resize(fifosize);
	};

	virtual ~SpherePackFifo(void) = default;
...
public:
	...
	SpherePack * AddSphere_(const Vector3d & pos, float radius, void * userdata, bool isSphere, int flags = SPF_LEAF_TREE);
...
void SpherePack::NewPos(const Vector3d & pos)
{
	...
	{
		const float dist = DistanceSquared(mParent); // compute squared distance to our parent.
...
void SpherePack::NewPosRadius(const Vector3d & pos, float radius)
{
	...
	if (mParent && !HasSpherePackFlag(SPF_INTEGRATE))
	{
		...
		const float dist = DistanceSquared(mParent);
