...
BOOL CEffectMesh::__LoadData_Ver002(int iSize, const uint8_t * c_pbBuf)
{
	...
	for (auto n = 0; n < m_iGeomCount; ++n)
	{
		...
		if ("ifl" == strExtension)
		{
			...
			if (CEterPackManager::Instance().Get(File, pMeshData->szDiffuseMapFileName, &pMotionData))
			{
				...
				for (uint32_t i = 0; i < textFileLoader.GetLineCount(); ++i)
				{
					...
					auto * pImage =
						msl::inherit_cast<CGraphicImage *>(CResourceManager::Instance().GetResourcePointer(strTextureFileName.c_str()));
		...
		{
			auto * pImage =
				msl::inherit_cast<CGraphicImage *>(CResourceManager::Instance().GetResourcePointer(pMeshData->szDiffuseMapFileName));
...
CEffectMesh::~CEffectMesh() = default;
...
{
	return dwMeshIndex < m_MeshDataVector.size();
}
...
{
	return 0 != m_strMeshFileName.length();
}
...
CEffectMeshScript::CEffectMeshScript() = default;
CEffectMeshScript::~CEffectMeshScript() = default;
