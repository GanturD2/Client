...
void CEffectMeshInstance::OnRender()
{
	...
	for (uint32_t i = 0; i < pEffectMesh->GetMeshCount(); ++i)
	{
		...
		STATEMANAGER.DrawPrimitiveUP(D3DPT_TRIANGLELIST, rFrameData.dwIndexCount / 3, &rFrameData.PDTVertexVector[0], sizeof(TPTVertex));
		// Render //
	}
}
...
{
	auto * pMesh = msl::inherit_cast<CEffectMeshScript *>(pElement);
	m_pMeshScript = pMesh;

	const char * c_szMeshFileName = pMesh->GetMeshFileName();

	m_pEffectMesh = msl::inherit_cast<CEffectMesh *>(CResourceManager::Instance().GetResourcePointer(c_szMeshFileName));
