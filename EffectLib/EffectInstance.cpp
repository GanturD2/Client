...
bool CEffectInstance::LessRenderOrder(CEffectInstance * pkEftInst) const
{
...
struct FEffectUpdator
{
	...
	explicit FEffectUpdator(float fElapsedTime) : isAlive(FALSE), fElapsedTime(fElapsedTime) {}
...
void CEffectInstance::SetSpecialRender() noexcept
{
...
BOOL CEffectInstance::isAlive() const
{
	return m_isAlive;
}
...
void CEffectInstance::SetEffectDataPointer(CEffectData* pEffectData, uint32_t* dwSkillColor, uint32_t EffectID)
#else
		...
		if (dwSkillColor != nullptr)
		{
			...
			if (skill != 99999999 && skill != 0)
			{
				...
				for (TTimeEventTableColor::iterator it = prob->m_TimeEventColor.begin(); it != prob->m_TimeEventColor.end(); ++it)
				{
					...
					c.a = d.a;
					it->m_Value.m_dwColor = (uint32_t)c;
				}
			}
			else if (skill == 0)
			{
