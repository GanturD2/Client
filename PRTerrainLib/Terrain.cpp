...
{
	msl::refill(m_lWaterHeight, -1);

	m_byNumWater = 0;
	m_HeightMapHeader = {};
	msl::refill(m_awShadowMap, 0xFFFF);
...
bool CTerrainImpl::LoadHeightMap(const char * c_szFileName)
{
	...
	memcpy(m_awRawHeightMap, lpcvFileData, sizeof(uint16_t) * HEIGHTMAP_RAW_XSIZE * HEIGHTMAP_RAW_YSIZE);
...
bool CTerrainImpl::LoadAttrMap(const char * c_szFileName)
{
	...
	uint32_t dwFileSize = kMappedFile.Size();
	auto * abFileData = (uint8_t *) lpcvFileData;
...
#pragma pack(pop)
		...
		{
			TraceError("CTerrainImpl::LoadAttrMap - %s MAGIC NUMBER(%d!=MAGIC[%d]) ERROR", c_szFileName, kAttrMapHeader.m_wMagic,
		...
		{
			TraceError("CTerrainImpl::LoadAttrMap - kAttrMapHeader(%s).m_width(%d)!=ATTRMAP_XSIZE(%d)", c_szFileName,
					   kAttrMapHeader.m_wWidth, ATTRMAP_XSIZE);
			return false;
		}

		if (ATTRMAP_YSIZE != kAttrMapHeader.m_wHeight)
		{
			TraceError("CTerrainImpl::LoadAttrMap - kAttrMapHeader(%s).m_height(%d)!=ATTRMAP_YSIZE(%d)", c_szFileName,
					   kAttrMapHeader.m_wHeight, ATTRMAP_YSIZE);
			return false;
		}
		...
		{
			TraceError("CTerrainImpl::LoadAttrMap - %s FILE DATA SIZE(rest %d != need %d) ERROR", c_szFileName, dwFileRestSize,
					   dwFileNeedSize);
			return false;
		}
...
bool CTerrainImpl::RAW_LoadTileMap(const char * c_szFileName)
{
	...
	memcpy(m_abyTileMap, lpcvFileData, sizeof(uint8_t) * (TILEMAP_RAW_XSIZE) * (TILEMAP_RAW_YSIZE));
...
#pragma pack(pop)
		...
		{
			TraceError("CTerrainImpl::LoadWaterMap - %s MAGIC NUMBER(%d!=MAGIC[%d]) ERROR", c_szFileName, kWaterMapHeader.m_wMagic,
					   c_wWaterMapMagic);
			return false;
		}
		...
		{
			TraceError("CTerrainImpl::LoadWaterMap - kWaterMapHeader(%s).m_width(%d)!=WATERMAP_XSIZE(%d)", c_szFileName,
					   kWaterMapHeader.m_wWidth, WATERMAP_XSIZE);
			return false;
		}

		if (WATERMAP_YSIZE != kWaterMapHeader.m_wHeight)
		{
			TraceError("CTerrainImpl::LoadWaterMap - kWaterMapHeader(%s).m_height(%d)!=WATERMAP_YSIZE(%d)", c_szFileName,
					   kWaterMapHeader.m_wHeight, WATERMAP_YSIZE);
			return false;
		}
		...
		uint32_t dwFileNeedSize = sizeof(m_abyWaterMap) + sizeof(long) * m_byNumWater;
		...
		{
			TraceError("CTerrainImpl::LoadWaterMap - %s FILE DATA SIZE(rest %d != need %d) ERROR", c_szFileName, dwFileRestSize,
					   dwFileNeedSize);
			return false;
		}
