...
void CTextureSet::Create()
{
	...
	m_ErrorTexture.ImageInstance.SetImagePointer(msl::inherit_cast<CGraphicImage *>(pResource));
...
bool CTextureSet::SetTexture(unsigned long ulIndex, const char * c_szFileName, float fuScale, float fvScale, float fuOffset, float fvOffset,
							 bool bSplat, uint16_t usBegin, uint16_t usEnd, float fTerrainTexCoordBase)
{
	...
	tex.Begin = usBegin;
	tex.End = usEnd;
	tex.ImageInstance.SetImagePointer(msl::inherit_cast<CGraphicImage *>(pResource));
...
void CTextureSet::Reload(float fTerrainTexCoordBase)
{
	for (uint32_t dwIndex = 1; dwIndex < GetTextureCount(); ++dwIndex)
	{
		...
		tex.ImageInstance.ReloadImagePointer(
			msl::inherit_cast<CGraphicImage *>(CResourceManager::Instance().GetResourcePointer(tex.stFilename.c_str())));
		tex.pd3dTexture = tex.ImageInstance.GetTexturePointer()->GetD3DTexture();
...
bool CTextureSet::AddTexture(const char * c_szFileName, float fuScale, float fvScale, float fuOffset, float fvOffset, bool bSplat,
							 uint16_t usBegin, uint16_t usEnd, float fTerrainTexCoordBase)
{
