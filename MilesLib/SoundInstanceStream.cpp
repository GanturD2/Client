...
void CSoundInstanceStream::Play(int count, uint32_t dwPlayCycleTimeLimit) const
{
	if (!IsData())
		return;
...
void CSoundInstanceStream::SetOrientation(float x_face, float y_face, float z_face, float x_normal, float y_normal, float z_normal) const {}
