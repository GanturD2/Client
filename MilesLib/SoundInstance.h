...
public:
	...
	virtual void Play(int iLoopCount = 1, uint32_t dwPlayCycleTimeLimit = 0) const = 0;
...
public: // from interface
	...
	bool SetSound(CSoundData * pSoundData);
	void Play(int iLoopCount = 1, uint32_t dwPlayCycleTimeLimit = 0) const;
	void Pause() const;
	void Resume() const;
	void Stop();
...
public: // from interface
	...
	void SetPosition(float x, float y, float z) const;
	void SetOrientation(float x_face, float y_face, float z_face, float x_normal, float y_normal, float z_normal) const;
	void SetVelocity(float fDistanceX, float fDistanceY, float fDistanceZ, float fNagnitude) const;
...
public: // from interface
	...
	void Play(int count = 1, uint32_t dwPlayCycleTimeLimit = 0) const;
	void Pause() const;
	void Resume() const;
	void Stop();
	void GetVolume(float & rfVolume) const;
