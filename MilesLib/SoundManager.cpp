...
CSoundManager::CSoundManager()
{
	...
	for (auto & rInstance : m_MusicInstances)
	{
...
void CSoundManager::UpdateSoundInstance(float fx, float fy, float fz, uint32_t dwcurFrame,
										const NSound::TSoundInstanceVector * c_pSoundInstanceVector, BOOL bCheckFrequency)
{
...
int CSoundManager::PlayAmbienceSound3D(float fx, float fy, float fz, const char * c_szFileName, int iPlayCount)
{
	...
	pInstance->SetPosition((fx - m_fxPosition) / m_fAmbienceSoundScale, (fy - m_fyPosition) / m_fAmbienceSoundScale,
...
void CSoundManager::FadeOutAllMusic()
{
	...
	for (auto & m_MusicInstance : m_MusicInstances)
	{
		if (MUSIC_STATE_OFF == m_MusicInstance.MusicState)
			continue;

		m_MusicInstance.MusicState = MUSIC_STATE_FADE_OUT;
		m_MusicInstance.fVolumeSpeed = 0.01f;
	}
}
