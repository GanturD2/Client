...
bool CNetworkDatagram::Create(uint32_t uPort)
{
...
int CNetworkDatagram::PeekRecvFrom(uint32_t uBufLen, void * pvBuf, SOCKADDR_IN * pkSockAddrIn)
{
...
int CNetworkDatagram::RecvFrom(uint32_t uBufLen, void * pvBuf, SOCKADDR_IN * pkSockAddrIn)
{
	int nSockAddrInLen = sizeof(SOCKADDR_IN);
	return recvfrom(m_sock, (char *) pvBuf, uBufLen, 0, (PSOCKADDR) pkSockAddrIn, &nSockAddrInLen);
}

int CNetworkDatagram::SendTo(uint32_t uBufLen, const void * c_pvBuf, const SOCKADDR_IN & c_rkSockAddrIn)
{
