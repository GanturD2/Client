...
bool CGraphicImageTexture::CreateFromMemoryFile(uint32_t bufSize, const void * c_pvBuf, D3DFORMAT d3dFmt, uint32_t dwFilter)
{
	...
	if (image.LoadHeaderFromMemory((const uint8_t *) c_pvBuf)) // DDS인가 확인
	...
	else
	{
		...
		if (FAILED(D3DXCreateTextureFromFileInMemoryEx(ms_lpd3dDevice, c_pvBuf, bufSize, imageInfo.Width, imageInfo.Height, D3DX_DEFAULT, 0,
													   d3dFmt, D3DPOOL_MANAGED, dwFilter, dwFilter, 0xffff00ff, nullptr, nullptr,
													   &m_lpd3dTexture)))
		{
		...
		if (IsLowTextureMemory())
			if (uTexBias || format != imageInfo.Format)
			{
				...
				if (SUCCEEDED(D3DXCreateTexture(ms_lpd3dDevice, imageInfo.Width >> uTexBias, imageInfo.Height >> uTexBias,
