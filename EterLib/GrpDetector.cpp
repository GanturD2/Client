...
{
	FIsEqualD3DDisplayMode(D3DDISPLAYMODE * pkD3DDMChk) { m_pkD3DDMChk = pkD3DDMChk; }
...
{
	auto * p1 = (D3DDISPLAYMODE *) arg1;
	auto * p2 = (D3DDISPLAYMODE *) arg2;
...
VOID D3D_SModeInfo::GetString(std::string * pstEnumList)
{
	...
	static const char * szVP[4] = {"UNKNOWN", "HWVP", "MXVP", "SWVP"};
...
BOOL D3D_CDeviceInfo::FindDepthStencilFormat(IDirect3D9 & rkD3D, uint32_t iD3DAdapterInfo, D3DDEVTYPE DeviceType, D3DFORMAT TargetFormat,
											 D3DFORMAT * pDepthStencilFormat)
{
	...
	{
		if (SUCCEEDED(
				rkD3D.CheckDeviceFormat(iD3DAdapterInfo, DeviceType, TargetFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D16)))
		{
	...
	{
		if (SUCCEEDED(
				rkD3D.CheckDeviceFormat(iD3DAdapterInfo, DeviceType, TargetFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D15S1)))
		{
	...
	{
		if (SUCCEEDED(
				rkD3D.CheckDeviceFormat(iD3DAdapterInfo, DeviceType, TargetFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D24X8)))
		{
	...
	{
		if (SUCCEEDED(
				rkD3D.CheckDeviceFormat(iD3DAdapterInfo, DeviceType, TargetFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D24S8)))
		{
	...
	{
		if (SUCCEEDED(rkD3D.CheckDeviceFormat(iD3DAdapterInfo, DeviceType, TargetFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE,
											  D3DFMT_D24X4S4)))
		{
	...
	{
		if (SUCCEEDED(
				rkD3D.CheckDeviceFormat(iD3DAdapterInfo, DeviceType, TargetFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, D3DFMT_D32)))
		{
...
BOOL D3D_CDeviceInfo::Build(IDirect3D9 & rkD3D, uint32_t iD3DAdapterInfo, uint32_t iDevType, D3D_CAdapterDisplayModeList & rkD3DADMList,
							BOOL (*pfnConfirmDevice)(D3DCAPS9 & rkD3DCaps, uint32_t uBehavior, D3DFORMAT eD3DFmt))
{
...
BOOL D3D_CAdapterInfo::Find(uint32_t uScrWidth, uint32_t uScrHeight, uint32_t uScrDepthBits, BOOL isWindowed, uint32_t * piD3DModeInfo,
							uint32_t * piD3DDevInfo)
{
...
D3D_CDisplayModeAutoDetector::~D3D_CDisplayModeAutoDetector() = default;
...
BOOL D3D_CDisplayModeAutoDetector::Find(uint32_t uScrWidth, uint32_t uScrHeight, uint32_t uScrDepthBits, BOOL isWindowed,
										uint32_t * piD3DModeInfo, uint32_t * piD3DDevInfo, uint32_t * piD3DAdapterInfo)
{
...
BOOL D3D_CDisplayModeAutoDetector::Build(IDirect3D9 & rkD3D, PFNCONFIRMDEVICE pfnConfirmDevice)
{
	...
	auto uTotalAdapterCount = rkD3D.GetAdapterCount();
	uTotalAdapterCount = std::min<UINT>(uTotalAdapterCount, D3DADAPTERINFO_NUM);

	for (auto iD3DAdapterInfo = 0; iD3DAdapterInfo < uTotalAdapterCount; ++iD3DAdapterInfo)
	{
