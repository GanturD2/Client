...
bool CGraphicCollisionObject::IntersectBoundBox(const D3DXMATRIX * c_pmatWorld, const TBoundBox & c_rboundBox, float * pu, float * pv,
												float * pt)
{
	return IntersectCube(c_pmatWorld, c_rboundBox.sx, c_rboundBox.sy, c_rboundBox.sz, c_rboundBox.ex, c_rboundBox.ey, c_rboundBox.ez,
						 ms_vtPickRayOrig, ms_vtPickRayDir, pu, pv, pt);
}
											...
											D3DXVECTOR3 & RayOriginal, D3DXVECTOR3 & RayDirection, float * pu, float * pv, float * pt)
{
	...
	return IntersectIndexedMesh(c_pmatWorld, posVertices, sizeof(TPosition), 8, c_awFillCubeIndices, 36, RayOriginal, RayDirection, pu, pv,
								pt);
}

const int c_iLimitVertexCount = 1024;

bool CGraphicCollisionObject::IntersectIndexedMesh(const D3DXMATRIX * c_pmatWorld, const void * vertices, int step, int vtxCount,
												   ...
												   D3DXVECTOR3 & RayDirection, float * pu, float * pv, float * pt)
{
	...
	auto * pcurVtx = (char *) vertices;

	while (vtxCount--)
	{
		auto * pos = (float *) pcurVtx;
	...
	auto * pcurIdx = (uint16_t *) indices;

	int triCount = idxCount / 3;
	while (triCount--)
	{
		if (IntersectTriangle(RayOriginal, RayDirection, s_v3PositionArray[pcurIdx[0]], s_v3PositionArray[pcurIdx[1]],
...
bool CGraphicCollisionObject::IntersectMesh(const D3DXMATRIX * c_pmatWorld, const void * vertices, uint32_t dwStep, uint32_t dwvtxCount,
											D3DXVECTOR3 & RayOriginal, D3DXVECTOR3 & RayDirection, float * pu, float * pv, float * pt)
{
	auto * pcurVtx = (char *) vertices;
...
bool CGraphicCollisionObject::IntersectTriangle(const D3DXVECTOR3 & c_orig, const D3DXVECTOR3 & c_dir, const D3DXVECTOR3 & c_v0,
												const D3DXVECTOR3 & c_v1, const D3DXVECTOR3 & c_v2, float * pu, float * pv, float * pt)
{
...
bool CGraphicCollisionObject::IntersectSphere(const D3DXVECTOR3 & c_rv3Position, float fRadius, const D3DXVECTOR3 & c_rv3RayOriginal,
...
CGraphicCollisionObject::CGraphicCollisionObject() = default;
