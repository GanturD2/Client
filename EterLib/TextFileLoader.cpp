...
CTextFileLoader * CTextFileLoader::Cache(const char * c_szFileName)
{
	...
	if (ms_kMap_dwNameKey_pkTextFileLoader.end() != f)
	{
		if (!ms_isCacheMode)
		{
			...
			auto * pkNewTextFileLoader = new CTextFileLoader;
...
bool CTextFileLoader::LoadGroup(TGroupNode * pGroupNode)
{
	...
	for (; m_dwcurLineIndex < m_textFileLoader.GetLineCount(); ++m_dwcurLineIndex)
	{
		...
		else
		{
			...
			{
				TraceError("CTextFileLoader::LoadGroup : must have a value (filename: %s line: %d key: %s)", m_strFileName.c_str(),
						   m_dwcurLineIndex, key.c_str());
				break;
			}
...
BOOL CTextFileLoader::GetTokenDoubleWord(const std::string & c_rstrKey, const uint32_t * pData)
{
