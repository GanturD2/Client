...
#define MAKEIMEVERSION(major, minor) ((uint32_t)(((uint8_t)(major) << 24) | ((uint8_t)(minor) << 16)))
#define IMEID_VER(dwId) ((dwId) &0xffff0000)
#define IMEID_LANG(dwId) ((dwId) &0x0000ffff)

#define GETLANG() LOWORD(CIME::ms_hklCurrent)
#define GETPRIMLANG() ((uint16_t) PRIMARYLANGID(GETLANG()))
...
#define IMEID_CHT_VER_VISTA \
	(LANG_CHT | MAKEIMEVERSION(7, 0)) // All TSF TIP under Cicero UI-less mode: a hack to make GetImeId() return non-zero value
...
wchar_t s_aszIndicator[5][3] = {L"En", L"\x7B80", L"\x7E41", L"\xAC00", L"\x3042"};

INPUTCONTEXT *(WINAPI * CIME::_ImmLockIMC)(HIMC);
BOOL(WINAPI * CIME::_ImmUnlockIMC)
(HIMC);
LPVOID(WINAPI * CIME::_ImmLockIMCC)
(HIMCC);
BOOL(WINAPI * CIME::_ImmUnlockIMCC)
(HIMCC);

uint32_t(WINAPI * CIME::_GetReadingString)(HIMC, uint32_t, LPWSTR, PINT, BOOL *, PUINT);
BOOL(WINAPI * CIME::_ShowReadingWindow)
(HIMC, BOOL);
...
uint32_t CIME::ms_dwImeState;

uint32_t CIME::ms_adwId[2] = {0, 0};

// IME Level
uint32_t CIME::ms_dwIMELevel;
uint32_t CIME::ms_dwIMELevelSaved;

// Candidate List
bool CIME::ms_bCandidateList;
uint32_t CIME::ms_dwCandidateCount;
...
uint32_t CIME::ms_dwCandidateSelection;
uint32_t CIME::ms_dwCandidatePageSize;
...
uint32_t CIME::ms_uOutputCodePage = 0;
uint32_t CIME::ms_uInputCodePage = 0;

extern uint32_t gs_codePage = 0;
...
protected:
	...
	public:
		...
		STDMETHODIMP BeginUIElement(uint32_t dwUIElementId, BOOL * pbShow) override;
		STDMETHODIMP UpdateUIElement(uint32_t dwUIElementId) override;
		STDMETHODIMP EndUIElement(uint32_t dwUIElementId) override;

		// ITfInputProcessorProfileActivationSink
		//   Notification for keyboard input locale change
		STDMETHODIMP OnActivated(uint32_t dwProfileType, LANGID langid, REFCLSID clsid, REFGUID catid, REFGUID guidProfile, HKL hkl,
	...
	static ITfUIElement * GetUIElement(uint32_t dwUIElementId);
	static BOOL GetCompartments(ITfCompartmentMgr ** ppcm, ITfCompartment ** ppTfOpenMode, ITfCompartment ** ppTfConvMode);
	static BOOL SetupCompartmentSinks(BOOL bRemoveOnly = FALSE, ITfCompartment * pTfOpenMode = nullptr,
									  ITfCompartment * pTfConvMode = nullptr);

	static ITfThreadMgrEx * m_tm;
	static uint32_t m_dwUIElementSinkCookie;
	static uint32_t m_dwAlpnSinkCookie;
	static uint32_t m_dwOpenModeSinkCookie;
	static uint32_t m_dwConvModeSinkCookie;
...
uint32_t CTsfUiLessMode::m_dwUIElementSinkCookie = TF_INVALID_COOKIE;
uint32_t CTsfUiLessMode::m_dwAlpnSinkCookie = TF_INVALID_COOKIE;
uint32_t CTsfUiLessMode::m_dwOpenModeSinkCookie = TF_INVALID_COOKIE;
uint32_t CTsfUiLessMode::m_dwConvModeSinkCookie = TF_INVALID_COOKIE;
...
public:
	...
	void Initialize()
	{
		...
		{
			m_bComInit = true;
			hr = CoCreateInstance(CLSID_TF_ThreadMgr, nullptr, CLSCTX_INPROC_SERVER, __uuidof(ITfThreadMgr), (void **) &m_ptim);
		}
	}
	void Uninitialize()
	{
...
private:
	ITfThreadMgr * m_ptim{nullptr};
	bool m_bComInit{false};
};
...
bool CIME::Initialize(HWND hWnd)
{
	...
	if (ms_stOSVI.dwPlatformId == VER_PLATFORM_WIN32_NT || (ms_stOSVI.dwMajorVersion > 4) ||
	...
	if (GetSystemDirectoryA(szPath, MAX_PATH + 1))
	{
		...
		if (ms_hImm32Dll)
		{
			...
			auto _ImmDisableTextFrameService = (BOOL(WINAPI *)(uint32_t)) GetProcAddress(ms_hImm32Dll, "ImmDisableTextFrameService");
			if (_ImmDisableTextFrameService)
				_ImmDisableTextFrameService((uint32_t) -1);
		}
		else
...
int CIME::GetReading(std::string & rstrText)
{
	...
	if (ms_wstrReading.empty())
		return 0;
	int readingLen =
		WideCharToMultiByte(ms_uOutputCodePage, 0, &ms_wstrReading[0], ms_wstrReading.size(), reading, sizeof(reading), nullptr, nullptr);
...
int CIME::GetText(std::string & rstrText, bool addCodePage)
{
	...
	len +=
		WideCharToMultiByte(dataCodePage, 0, m_wText + ms_curpos, ms_lastpos - ms_curpos, text + len, sizeof(text) - len, nullptr, nullptr);
...
int CIME::GetCandidate(uint32_t index, std::string & rstrText)
{
	...
	char text[IMESTR_MAXLEN];
	int len = ::WideCharToMultiByte(CP_UTF8, 0, wszText, wTextLen, text, sizeof(text), nullptr, nullptr);
...
void CIME::SetInputMode(uint32_t dwMode)
{
...
uint32_t CIME::GetInputMode()
{
	unsigned long dwCMode, dwSMode;
...
void CIME::PasteTextFromClipBoard()
{
	...
	const int wstrLen = MultiByteToWideChar(ms_uInputCodePage, 0, begin, end - begin, m_wText, IMESTR_MAXLEN);
...
{
	const auto uLanguage = (uint32_t) GETLANG();
	CheckToggleState();
	ChangeInputLanguageWorker();
...
void CIME::SetSupportLevel(uint32_t dwImeLevel)
{
...
void CIME::IncCurPos()
{
	...
	{
		const int pos = FindColorTagEndPosition(m_wText + ms_curpos, ms_lastpos - ms_curpos);
		if (pos > 0)
...
int CIME::GetCurPos()
{
	const int pos = GetTextTagOutputLen(m_wText, ms_curpos);
	return pos;
}
...
void CIME::DelCurPos()
{
	if (ms_curpos < ms_lastpos)
	{
		const int eraseCount = FindColorTagEndPosition(m_wText + ms_curpos, ms_lastpos - ms_curpos) + 1;
...
uint32_t CIME::GetCodePageFromLang(LANGID langid)
{
	const unsigned pri_langid = PRIMARYLANGID(langid);
	switch (pri_langid)
	{
	case LANG_JAPANESE:
		return 932;
...
{
	const int textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, m_wText, ms_lastpos, nullptr, 0, nullptr, nullptr);
	if (textLen >= m_max)
	{
		ImmNotifyIME(hImc, NI_COMPOSITIONSTR, CPS_CANCEL, 0);
		ms_compLen = 0;
...
void CIME::ResultProcess(HIMC hImc)
{
	...
	const int len = ImmGetCompositionStringW(hImc, GCS_RESULTSTR, temp, sizeof(temp)) / sizeof(wchar_t);
	if (len <= 0)
		return;
...
{
	uint8_t attribute[IMESTR_MAXLEN];
	const LONG attributeLen = ImmGetCompositionStringW(hImc, GCS_COMPATTR, &attribute, sizeof(attribute)) / sizeof(uint8_t);
...
{
	std::vector<uint8_t> abyCandidate;
	uint32_t dwCandidateLen = ImmGetCandidateListW(hImc, 0, nullptr, 0);
	...
	if (dwCandidateLen > 0)
	{
		...
		if (GETLANG() == LANG_CHS)
		{
			...
			{
				const uint32_t uLen = lstrlenW((LPWSTR)((uint32_t) lpCandidateList + lpCandidateList->dwOffset[i])) + (3 - sizeof(WCHAR));
		...
		else
		{
			...
			iStartOfPage =
				ms_bUILessMode ? lpCandidateList->dwPageStart : (ms_dwCandidateSelection / (MAX_CANDLIST - 1)) * (MAX_CANDLIST - 1);
		}

		ms_dwCandidateSelection = (GETLANG() == LANG_CHS && !GetImeId()) ? (uint32_t) -1 : ms_dwCandidateSelection - iStartOfPage;

		//printf( "SEL: %d, START: %d, PAGED: %d\n", ms_dwCandidateSelection, iStartOfPage, ms_dwCandidatePageSize );
		memset(&ms_wszCandidate, 0, sizeof(ms_wszCandidate));
		for (uint32_t i = iStartOfPage, j = 0; (uint32_t) i < lpCandidateList->dwCount && j < ms_dwCandidatePageSize; i++, j++)
			wcscpy(ms_wszCandidate[j], (LPWSTR)((uint32_t) lpCandidateList + lpCandidateList->dwOffset[i]));
		...
		if (GETPRIMLANG() == LANG_KOREAN || GETLANG() == LANG_CHT && !GetImeId())
			ms_dwCandidateSelection = (uint32_t) -1;
	}
}
	...
	else
	{
		...
		else
		{
			...
			case IMEID_CHT_VER44: // New ChanJie IME98b          : 4.4.x.x // WinXP
				...
				if (!p)
					break;
				tempLen = *(uint32_t *) (p + 7 * 4 + 32 * 4);
				dwErr = *(uint32_t *) (p + 8 * 4 + 32 * 4);
				temp = (wchar_t *) (p + 56);
				bUnicodeIme = true;
				break;
			...
			case IMEID_CHT_VER50: // 5.0.x.x // WinME
				...
				if (!p)
					break;
				tempLen = *(uint32_t *) (p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16);
				dwErr = *(uint32_t *) (p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 + 1 * 4);
			...
			case IMEID_CHS_VER53: // 5.3.x.x // SCIME2k or MSPY3 (w/OfficeXP and Whistler)
				...
				if (!p)
					break;
				tempLen = *(uint32_t *) (p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 * 2);
				dwErr = *(uint32_t *) (p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 * 2 + 1 * 4);
				temp = (wchar_t *) (p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4);
				bUnicodeIme = true;
				break;
			...
			case IMEID_CHS_VER41:
			{
				...
				if (!p)
					break;
				tempLen = *(uint32_t *) (p + 7 * 4 + 16 * 2 * 4);
				dwErr = *(uint32_t *) (p + 8 * 4 + 16 * 2 * 4);
			...
			case IMEID_CHS_VER42: // 4.2.x.x // SCIME98 or MSPY2 (w/Office2k, Win2k, WinME, etc)
			{
				...
				const int nTcharSize = (osi.dwPlatformId == VER_PLATFORM_WIN32_NT) ? sizeof(wchar_t) : sizeof(char);
				p = *(LPBYTE *) ((LPBYTE) _ImmLockIMCC(lpIC->hPrivate) + 1 * 4 + 1 * 4 + 6 * 4);
				if (!p)
					break;
				tempLen = *(uint32_t *) (p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 * nTcharSize);
				dwErr = *(uint32_t *) (p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4 + 16 * nTcharSize + 1 * 4);
				temp = (wchar_t *) (p + 1 * 4 + (16 * 2 + 2 * 4) + 5 * 4);
...
bool CIME::IsMax(const wchar_t * wInput, int len)
{
	...
	const int textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, m_wText, ms_lastpos, nullptr, 0, nullptr, nullptr);
	const int inputLen = WideCharToMultiByte(ms_uOutputCodePage, 0, wInput, len, nullptr, 0, nullptr, nullptr);
	...
	else if (m_userMax != 0 && m_max != m_userMax)
	{
		...
		const int textLen = WideCharToMultiByte(ms_uOutputCodePage, 0, str.c_str(), str.length(), nullptr, 0, nullptr, nullptr);
		const int inputLen = WideCharToMultiByte(ms_uOutputCodePage, 0, input.c_str(), input.length(), nullptr, 0, nullptr, nullptr);
		return textLen + inputLen > m_userMax;
	}
	return false;
}

uint32_t CIME::GetImeId(uint32_t uIndex)
{
	static HKL hklPrev = nullptr;
	char szTmp[1024];
	...
	const uint32_t dwLang = ((uint32_t) hkl & 0xffff);
	...
	unsigned long dwVerHandle;
	const auto dwVerSize = GetFileVersionInfoSize(szTmp, &dwVerHandle);
	const LANGID langId = LOWORD(ms_hklCurrent);
	...
	if (dwVerSize)
	{
		...
		if (GetFileVersionInfo(szTmp, dwVerHandle, dwVerSize, lpVerBuffer))
		{
			...
			{
				uint32_t dwVer = ((VS_FIXEDFILEINFO *) lpVerData)->dwFileVersionMS;
				dwVer = (dwVer & 0x00ff0000) << 8 | (dwVer & 0x000000ff) << 16;

				if (_GetReadingString ||
					(langId == LANG_CHT &&
					 (dwVer == MAKEIMEVERSION(4, 2) || dwVer == MAKEIMEVERSION(4, 3) || dwVer == MAKEIMEVERSION(4, 4) ||
					  dwVer == MAKEIMEVERSION(5, 0) || dwVer == MAKEIMEVERSION(5, 1) || dwVer == MAKEIMEVERSION(5, 2) ||
					  dwVer == MAKEIMEVERSION(6, 0))) ||
					(langId == LANG_CHS &&
					 (dwVer == MAKEIMEVERSION(4, 1) || dwVer == MAKEIMEVERSION(4, 2) || dwVer == MAKEIMEVERSION(5, 3))))
				{
...
bool CIME::GetReadingWindowOrientation()
{
	...
	if (!bHorizontalReading && (GETLANG() == LANG_CHT))
	{
		...
		HKEY hKey;
		const uint32_t dwVer = ms_adwId[0] & 0xFFFF0000;
		strcpy(szRegPath, R"(software\microsoft\windows\currentversion\)");
		...
		{
			unsigned long dwSize = sizeof(unsigned long), dwMapping, dwType;
			lRc = RegQueryValueExA(hKey, "Keyboard Mapping", nullptr, &dwType, (PBYTE) &dwMapping, &dwSize);
			if (lRc == ERROR_SUCCESS)
			{
				if ((dwVer <= MAKEIMEVERSION(5, 0) && ((uint8_t) dwMapping == 0x22 || (uint8_t) dwMapping == 0x23)) ||
					((dwVer == MAKEIMEVERSION(5, 1) || dwVer == MAKEIMEVERSION(5, 2)) && (uint8_t) dwMapping >= 0x22 &&
					 (uint8_t) dwMapping <= 0x24))
...
void CIME::SetupImeApi()
{
	...
	if (ImmGetIMEFileNameA(ms_hklCurrent, szImeFile, COUNTOF(szImeFile) - 1) == 0)
		return;
	if (stricmp(szImeFile, CHS_IMEFILENAME_QQPINYIN) == 0 || stricmp(szImeFile, CHS_IMEFILENAME_SOGOUPY) == 0 ||
		stricmp(szImeFile, CHS_IMEFILENAME_GOOGLEPINYIN2) == 0)
	...
	{
		_GetReadingString =
			(uint32_t(WINAPI *)(HIMC, uint32_t, LPWSTR, PINT, BOOL *, PUINT))(GetProcAddress(ms_hCurrentImeDll, "GetReadingString"));
...
static unsigned long _strtoul(LPCSTR psz, LPTSTR * /*unused*/, int /*unused*/)
{
	if (!psz)
		return 0;
...
{
	static uint32_t s_uPrevCodePage = 0xFFFF;
	...
	char szCodePage[8];
	const int iRc = GetLocaleInfoA(MAKELCID(GETLANG(), SORT_DEFAULT), LOCALE_IDEFAULTANSICODEPAGE, szCodePage, COUNTOF(szCodePage));
	iRc;
	...
	//for ( int i = 0; i < 256; i++ )
	//{
	//	LeadByteTable[i] = (uint8_t)IsDBCSLeadByteEx( g_uCodePage, (uint8_t)i );
	//}
}
...
void CIME::CheckToggleState()
{
	...
	/* Check Toggle State */
	const bool bIme = ImmIsIME(ms_hklCurrent) != 0 &&
		((0xF0000000 & (uint32_t) ms_hklCurrent) ==
		 0xE0000000); // Hack to detect IME correctly. When IME is running as TIP, ImmIsIME() returns true for CHT US keyboard.
	ms_bChineseIME = (GETPRIMLANG() == LANG_CHINESE) && bIme;

	HIMC himc;
	if (nullptr != (himc = ImmGetContext(ms_hWnd)))
	{
		if (ms_bChineseIME)
		{
...
CTsfUiLessMode::CUIElementSink::~CUIElementSink() = default;
...
STDAPI CTsfUiLessMode::CUIElementSink::BeginUIElement(uint32_t dwUIElementId, BOOL * pbShow)
{
...
STDAPI CTsfUiLessMode::CUIElementSink::UpdateUIElement(uint32_t dwUIElementId)
{
	ITfUIElement * pElement = GetUIElement(dwUIElementId);
	if (!pElement)
		return E_INVALIDARG;
...
STDAPI CTsfUiLessMode::CUIElementSink::EndUIElement(uint32_t dwUIElementId)
{
	ITfUIElement * pElement = GetUIElement(dwUIElementId);
	if (!pElement)
		return E_INVALIDARG;
...
STDAPI CTsfUiLessMode::CUIElementSink::OnActivated(uint32_t dwProfileType, LANGID langid, REFCLSID clsid, REFGUID catid,
												   REFGUID guidProfile, HKL hkl, uint32_t dwFlags)
{
	...
	if (SUCCEEDED(preading->GetString(&bstr)))
	{
		if (bstr)
		{
			...
			if (fVertical)
			{
				...
				CIME::ms_dwCandidateSelection = CIME::ms_iReadingError ? CIME::ms_iReadingError - 1 : (uint32_t) -1;
...
void CTsfUiLessMode::MakeCandidateStrings(ITfCandidateListUIElement * pcandidate)
{
	uint32_t uIndex = 0;
	uint32_t uCount = 0;
	uint32_t uCurrentPage = 0;
	uint32_t uPageCnt = 0;
	uint32_t dwPageStart = 0;
	...
	CIME::ms_dwCandidateSelection = (uint32_t) uIndex;
	CIME::ms_dwCandidateCount = (uint32_t) uCount;
	CIME::ms_bCandidateList = true;

	pcandidate->GetPageIndex(nullptr, 0, &uPageCnt);
	if (uPageCnt > 0)
	{
		std::vector<uint32_t> IndexList(uPageCnt);
		pcandidate->GetPageIndex(IndexList.data(), uPageCnt, &uPageCnt);
		dwPageStart = IndexList[uCurrentPage];
		dwPageSize = (uCurrentPage < uPageCnt - 1) ? std::min(uCount, IndexList[uCurrentPage + 1]) - dwPageStart : uCount - dwPageStart;
	}

	CIME::ms_dwCandidatePageSize = std::min<int>(dwPageSize, CIME::MAX_CANDLIST);
	...
	for (uint32_t i = dwPageStart, j = 0; (uint32_t) i < CIME::ms_dwCandidateCount && j < CIME::ms_dwCandidatePageSize; i++, j++)
	{
	...
	if (GETPRIMLANG() == LANG_KOREAN)
		CIME::ms_dwCandidateSelection = (uint32_t) -1;
}

ITfUIElement * CTsfUiLessMode::GetUIElement(uint32_t dwUIElementId)
{
...
BOOL CTsfUiLessMode::CurrentInputLocaleIsIme()
{
	...
	hr = CoCreateInstance(CLSID_TF_InputProcessorProfiles, nullptr, CLSCTX_INPROC_SERVER, __uuidof(ITfInputProcessorProfiles),
						  (LPVOID *) &pProfiles);
	if (SUCCEEDED(hr))
	{
...
{
	if (m_tm == nullptr || (bEnable && m_dwUIElementSinkCookie != TF_INVALID_COOKIE) ||
...
BOOL CTsfUiLessMode::SetupCompartmentSinks(BOOL bRemoveOnly, ITfCompartment * pTfOpenMode, ITfCompartment * pTfConvMode)
{
	...
	if (SUCCEEDED(hr = pTfOpenMode->QueryInterface(IID_ITfSource, (void **) &srcOpenMode)))
	{
		...
		if (bRemoveOnly ||
			SUCCEEDED(
				hr = srcOpenMode->AdviseSink(IID_ITfCompartmentEventSink, (ITfCompartmentEventSink *) m_TsfSink, &m_dwOpenModeSinkCookie)))
		{
			...
			if (SUCCEEDED(hr = pTfConvMode->QueryInterface(IID_ITfSource, (void **) &srcConvMode)))
			{
				...
				// Setup sink for open mode (toggle state) change
				if (bRemoveOnly ||
					SUCCEEDED(hr = srcConvMode->AdviseSink(IID_ITfCompartmentEventSink, (ITfCompartmentEventSink *) m_TsfSink,
...
/* IME Message Handler */
LRESULT CIME::WMInputLanguage(HWND hWnd, uint32_t /*uiMsg*/, WPARAM /*wParam*/, LPARAM lParam)
{
	ChangeInputLanguage();
	return 0;
}

LRESULT CIME::WMStartComposition(HWND /*hWnd*/, uint32_t /*uiMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	return 1L;
}

LRESULT CIME::WMComposition(HWND hWnd, uint32_t /*uiMsg*/, WPARAM /*wParam*/, LPARAM lParam)
{
	LRESULT result = 0;
...
LRESULT CIME::WMEndComposition(HWND /*hWnd*/, uint32_t /*uiMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	ms_compLen = 0;
	ms_ulbegin = 0;
	ms_ulend = 0;
...
LRESULT CIME::WMNotify(HWND hWnd, uint32_t uiMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT result = 0;
...
LRESULT CIME::WMChar(HWND /*hWnd*/, uint32_t /*uiMsg*/, WPARAM wParam, LPARAM lParam)
{
	const unsigned char c = unsigned char(wParam & 0xff);
