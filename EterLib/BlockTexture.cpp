...
{
	RECT dstRect = m_rect;
	if (c_rsrcRect.right < dstRect.left || c_rsrcRect.left > dstRect.right || c_rsrcRect.bottom < dstRect.top ||
	...
	const RECT clipRect = {std::max(c_rsrcRect.left - dstRect.left, 0l), std::max(c_rsrcRect.top - dstRect.top, 0l),
	...
	auto * pdwDst = (uint32_t *) lockedRect.pBits;
