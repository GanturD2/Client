...
void CScreen::RenderBar3d(float sx, float sy, float sz, float ex, float ey, float ez)
{
	...
	SPDTVertexRaw vertices[4] = {{sx, sy, sz, ms_diffuseColor, 0.0f, 0.0f},
...
void CScreen::RenderBar3d(const D3DXVECTOR3 * c_pv3Positions)
{
	...
	SPDTVertexRaw vertices[4] = {{c_pv3Positions[0].x, c_pv3Positions[0].y, c_pv3Positions[0].z, ms_diffuseColor, 0.0f, 0.0f},
...
void CScreen::RenderGradationBar3d(float sx, float sy, float sz, float ex, float ey, float ez, uint32_t dwStartColor, uint32_t dwEndColor)
{
	...
	SPDTVertexRaw vertices[4] = {{sx, sy, sz, dwStartColor, 0.0f, 0.0f},
...
{
	SPDTVertexRaw vertices[8] = {{sx, sy, sz, ms_diffuseColor, 0.0f, 0.0f}, {ex, sy, sz, ms_diffuseColor, 0.0f, 0.0f},
...
void CScreen::RenderCube(float sx, float sy, float sz, float ex, float ey, float ez, D3DXMATRIX matRotation)
{
	...
	D3DXVECTOR3 v3Vertex[8] = {D3DXVECTOR3(sx, sy, sz), D3DXVECTOR3(ex, sy, sz), D3DXVECTOR3(sx, ey, sz), D3DXVECTOR3(ex, ey, sz),
							   D3DXVECTOR3(sx, sy, ez), D3DXVECTOR3(ex, sy, ez), D3DXVECTOR3(sx, ey, ez), D3DXVECTOR3(ex, ey, ez)};
	SPDTVertexRaw vertices[8];
...
void CScreen::RenderCircle3d(float fx, float fy, float fz, float fRadius, int iStep)
{
	...
	for (count = 0; count < iStep - 1; count++)
	{
		RenderLine3d(fx + pts[count].x, fy + pts[count].y, fz + pts[count].z, fx + pts[count + 1].x, fy + pts[count + 1].y,
					 fz + pts[count + 1].z);
	}
...
public:
	...
	~CD3DXMeshRenderingOption() override
	{
...
void CScreen::RenderD3DXMesh(LPD3DXMESH lpMesh, const D3DXMATRIX * c_pmatWorld, float fx, float fy, float fz, float fRadius,
...
void CScreen::RenderCylinder(const D3DXMATRIX * c_pmatWorld, float fx, float fy, float fz, float fRadius, float /*fLength*/,
							 D3DFILLMODE d3dFillMode)
{
...
void CScreen::DrawGrid(float xMin, float yMin, float xMax, float yMax, float xmajorStep, float ymajorStep, float xminorStep,
...
bool CScreen::GetCursorXYPosition(float * px, float * py)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		if (IntersectTriangle(ms_vtPickRayOrig, ms_vtPickRayDir, posVertices[sc_awFillRectIndices[i * 3 + 0]],
							  posVertices[sc_awFillRectIndices[i * 3 + 1]], posVertices[sc_awFillRectIndices[i * 3 + 2]], &u, &v, &t))
		{
...
void CScreen::BuildViewFrustum()
{
	...
	D3DXMATRIX vv = ms_matView * ms_matProj;
