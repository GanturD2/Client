...
public:
	...
	virtual ~CDynamicPool() { assert(m_kVct_pkData.empty()); }
	...
	void FreeAll() { m_kVct_pkFree = m_kVct_pkData; }

	uint32_t GetCapacity() { return m_kVct_pkData.size(); }
...
protected:
	...
	static void Delete(T * pkData) { delete pkData; }
...
template <typename T> class CDynamicPoolEx
{
public:
	CDynamicPoolEx()
	{
...
protected:
	...
	static T * New() { return (T *) ::operator new(sizeof(T)); }
	static void Delete(T * pkData) { ::operator delete(pkData); }
...
template <class T> class CPooledObject
{
public:
	CPooledObject() {}
	virtual ~CPooledObject() {}

	void * operator new(unsigned int /*mem_size*/) { return ms_kPool.Alloc(); }

	void operator delete(void * pT) { ms_kPool.Free((T *) pT); }


	static void DestroySystem() { ms_kPool.Destroy(); }

	static void DeleteAll() { ms_kPool.FreeAll(); }
