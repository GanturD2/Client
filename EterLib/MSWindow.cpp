...
{
	auto * pWnd = (CMSWindow *) GetWindowLong(hWnd, GWL_USERDATA);
...
bool CMSWindow::Create(const char * c_szName, int brush, uint32_t cs, uint32_t ws, HICON hIcon, int iCursorResource)
{
	...
	m_hWnd = CreateWindow(c_szClassName, c_szName, ws, 0, 0, 0, 0, nullptr, nullptr, ms_hInstance, nullptr);
...
void CMSWindow::AdjustSize(int width, int height)
{
	...
	AdjustWindowRectEx(&m_rect, GetWindowStyle(m_hWnd), GetMenu(m_hWnd) != nullptr, GetWindowExStyle(m_hWnd));
...
const char * CMSWindow::RegisterWindowClass(uint32_t style, int brush, WNDPROC pfnWndProc, HICON hIcon, int iCursorResource)
{
	...
	ms_stWCSet.emplace((char * const) c_szStaticClassName);
