...
void CDecal::Clear()
{
	...
	msl::refill(m_Vertices);
	msl::refill(m_Indices);
}
...
uint32_t CDecal::ClipPolygon(uint32_t dwVertexCount, const D3DXVECTOR3 * c_pv3Vertex, const D3DXVECTOR3 * c_pv3Normal,
							 D3DXVECTOR3 * c_pv3NewVertex, D3DXVECTOR3 * c_pv3NewNormal) const
{
		...
		if (dwCount != 0)
		{
			...
			if (dwCount != 0)
			{
				...
				if (dwCount != 0)
				{
					...
					if (dwCount != 0)
						dwCount =
							ClipPolygonAgainstPlane(m_v4FrontPlane, dwCount, v3TempVertex, v3TempNormal, c_pv3NewVertex, c_pv3NewNormal);
				}
			}
		}
	}
...
uint32_t CDecal::ClipPolygonAgainstPlane(const D3DXPLANE & c_rv4Plane, uint32_t dwVertexCount, const D3DXVECTOR3 * c_pv3Vertex,
										 const D3DXVECTOR3 * c_pv3Normal, D3DXVECTOR3 * c_pv3NewVertex, D3DXVECTOR3 * c_pv3NewNormal)
{
	bool bNegative[10];
		...
		if (bNegative[dwCurIndex])
		{
			if (!bNegative[dwPrevIndex])
			{
				...
				float ft = D3DXPlaneDotCoord(&c_rv4Plane, &v3_1) /
					(c_rv4Plane.a * (v3_1.x - v3_2.x) + c_rv4Plane.b * (v3_1.y - v3_2.y) + c_rv4Plane.c * (v3_1.z - v3_2.z));
...
void CDecal::Render()
{
	...
	for (auto & dwi : m_TriangleFanStructVector)
		STATEMANAGER.DrawIndexedPrimitiveUP(D3DPT_TRIANGLEFAN, dwi.m_wMinIndex, dwi.m_dwVertexCount, dwi.m_dwPrimitiveCount,
											m_Indices + dwi.m_wMinIndex, D3DFMT_INDEX16, m_Vertices, sizeof(TPDTVertex));
}
...
{
	m_DecalPtrVector.emplace_back(pDecal);
}
