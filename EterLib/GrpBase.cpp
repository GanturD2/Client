...
{
	if (ms_dwD3DBehavior & D3DCREATE_HARDWARE_VERTEXPROCESSING || ms_dwD3DBehavior & D3DCREATE_MIXED_VERTEXPROCESSING)
	{
...
void CGraphicBase::SetEyeCamera(float xEye, float yEye, float zEye, float xCenter, float yCenter, float zCenter, float xUp, float yUp,
								float zUp)
{
...
float CGraphicBase::GetFOV() const noexcept
{
	return ms_fFieldOfView;
}

float CGraphicBase::GetAspect() const noexcept
{
	return ms_fAspect;
}

float CGraphicBase::GetNear() const noexcept
{
	return ms_fNearY;
}

float CGraphicBase::GetFar() const noexcept
{
	return ms_fFarY;
}
#else
...
{
	uint8_t argb[4] = {(uint8_t)(255.0f * b), (uint8_t)(255.0f * g), (uint8_t)(255.0f * r), (uint8_t)(255.0f * a)};
...
CGraphicBase::CGraphicBase() = default;
