...
PyObject * grpSaveScreenShotToPath(PyObject * poSelf, PyObject * poArgs)
{
	...
	snprintf(szPath, sizeof(szPath), "%s%02d%02d_%02d%02d%02d.jpg", szBasePath, tmNow->tm_mon + 1, tmNow->tm_mday, tmNow->tm_hour,
...
PyObject * grpSaveScreenShot(PyObject * poSelf, PyObject * poArgs)
{
	...
	sprintf(szPath + strlen(szPath), "%02d%02d_%02d%02d%02d.jpg", tmNow->tm_mon + 1, tmNow->tm_mday, tmNow->tm_hour, tmNow->tm_min,
			tmNow->tm_sec);
...
PyObject* grpSetDropItemOnOffLevel(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!CPythonGraphicOnOff::Instance().SetItemDropOnOffLevel(bLevel))
		return Py_BuildException();
...
{
	return Py_BuildValue("i", CPythonGraphicOnOff::Instance().GetItemDropOnOffLevel());
}

PyObject* grpSetPetOnOffStatus(PyObject* poSelf, PyObject* poArgs)
{
	uint8_t bStatus;
	if (!PyTuple_GetByte(poArgs, 0, &bStatus))
		return Py_BuildException();

	if (!CPythonGraphicOnOff::Instance().SetPetOnOffStatus(bStatus))
		return Py_BuildException();
...
{
	return Py_BuildValue("b", CPythonGraphicOnOff::Instance().GetPetOnOffStatus());
}
...
PyObject* grpSetNPCNameOnOffStatus(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!CPythonGraphicOnOff::Instance().SetNPCNameOnOffStatus(bStatus))
		return Py_BuildException();
...
{
	return Py_BuildValue("b", CPythonGraphicOnOff::Instance().GetNPCNameOnOffStatus());
}
#endif

void initgrp()
{
	static PyMethodDef s_methods[] = {{"InitScreenEffect", grpInitScreenEffect, METH_VARARGS},
