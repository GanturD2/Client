...
void CPythonGraphic::SetGamma(float fGammaFactor)
{
	...
	for (int i = 0; i < 256; ++i)
	{
		auto val = static_cast<int>(i * fGammaFactor * 255.0f);
		int ui = 0;
...
bool CPythonGraphic::SaveJPEG(const char * pszFileName, LPBYTE pbyBuffer, uint32_t uWidth, uint32_t uHeight)
{
...
bool CPythonGraphic::SaveScreenShot(const char * c_pszFileName)
{
	...
	std::vector<uint8_t> pbyBuffer;
	try
	{
		pbyBuffer.resize(uWidth * uHeight * 3);
	}
	catch (std::bad_alloc &)
	{
	...
	auto * pbySource = static_cast<uint8_t *>(lockRect.pBits);
	auto pbyDestination = pbyBuffer.data();
	for (uint32_t y = 0; y < uHeight; ++y)
	{
		...
		case D3DFMT_R5G6B5:
		{
			for (uint32_t x = 0; x < uWidth; ++x)
			{
				uint32_t uColor = *(reinterpret_cast<uint32_t *>(pRow));
	...
	bool bSaved = SaveJPEG(c_pszFileName, pbyBuffer.data(), uWidth, uHeight);
	...
	{
		msl::file_ptr srcFilePtr(c_pszFileName, "rb");
		if (srcFilePtr)
		{
			size_t fileSize = srcFilePtr.size();

			char head[21];
			size_t tailSize = fileSize - sizeof(head);
			std::vector<char> tail(tailSize);

			fread(head, sizeof(head), 1, srcFilePtr.get());
			fread(tail.data(), tailSize, 1, srcFilePtr.get());
			srcFilePtr.close();

			char imgDesc[64]{};
			GenScreenShotTag(c_pszFileName, GetCRC32(tail.data(), tailSize), imgDesc, sizeof(imgDesc));

			auto imgDescLen = msl::truncate_cast<uint8_t>(strlen(imgDesc) + 1);

			uint8_t exifHeader[] = {0xe1,
									0, // blockLen[1],
									0, // blockLen[0],
									0x45,       0x78, 0x69, 0x66, 0x0, 0x0, 0x49, 0x49, 0x2a, 0x0,
									0x8,        0x0,  0x0,  0x0,  0x1, 0x0, 0xe,  0x1,  0x2,  0x0,
			...
			msl::file_ptr dstFilePtr(c_pszFileName, "wb");
			if (dstFilePtr)
			{
				fwrite(head, sizeof(head), 1, dstFilePtr.get());
				fwrite(exifHeader, sizeof(exifHeader), 1, dstFilePtr.get());
				fwrite(imgDesc, imgDescLen, 1, dstFilePtr.get());
				fputc(0x00, dstFilePtr.get());
				fputc(0xff, dstFilePtr.get());
				fwrite(tail.data(), tailSize, 1, dstFilePtr.get());
			}
		}
	}
	return true;
...
void CPythonGraphic::RenderAlphaImage(CGraphicImageInstance * pImageInstance, float x, float y, float aLeft, float aRight) const
{
	assert(pImageInstance != nullptr);
...
void CPythonGraphic::RenderCoolTimeBoxColor(float fxCenter, float fyCenter, float fRadius, float fTime, float fColorR, float fColorG, float fColorB, float fColorA)
{
	if (fTime >= 1.0f)
	{
		return;
	...
	static D3DXCOLOR color = D3DXCOLOR(fColorR, fColorG, fColorB, fColorA);
...
#ifdef ENABLE_GROWTH_PET_SYSTEM
	static D3DXCOLOR color = D3DXCOLOR(fr, fg, fb, fa);
#else
	static D3DXCOLOR color = D3DXCOLOR(0.0f, 0.0f, 0.0f, 0.5f);
#endif
	static D3DXVECTOR2 s_v2BoxPos[8] = {D3DXVECTOR2(-1.0f, -1.0f), D3DXVECTOR2(-1.0f, 0.0f),  D3DXVECTOR2(-1.0f, +1.0f),
										D3DXVECTOR2(0.0f, +1.0f),  D3DXVECTOR2(+1.0f, +1.0f), D3DXVECTOR2(+1.0f, 0.0f),
...
void CPythonGraphic::RenderCoolTimeBoxInverse(float fxCenter, float fyCenter, float fRadius, float fTime)
{
	...
	float fLastPercentage = (8.0f * fTime) - iTriCount;
