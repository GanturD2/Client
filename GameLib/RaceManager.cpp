...
void __GetRaceResourcePathes(unsigned race, std::vector<std::string> & vec_stPathes)
{
	...
	else if (__IsNPCRace(race))
	{
		if (race >= 30000)
		{
			...
			if (race >= 34001 && race <= 34999)	//@fixme426
			{
				vec_stPathes.emplace_back("d:/ymir work/npc_pet/");
				vec_stPathes.emplace_back("d:/ymir work/npc2/");
			}
...
CRaceData * CRaceManager::__LoadRaceData(uint32_t dwRaceIndex)
{
	...
	for (int i = 0; i < vec_stFullPathName.size(); i++)
	{
		...
		if (!pRaceData->LoadRaceData(stMSMFileName.c_str()))
		{
			...
			{
				Tracenf("CRaceManager::RegisterRacePath : RACE[%u] LOAD MSMFILE[%s] ERROR. Will Find Another Path.", dwRaceIndex,
						stMSMFileName.c_str()); // @warme668
				continue;
			}
...
bool CRaceManager::__LoadRaceMotionList(CRaceData & rkRaceData, const char * pathName, const char * motionListFileName)
{
	...
	if (!s_isInit)
	{
		...
		s_kMap_stType_dwIndex.emplace("SPECIAL6", CRaceMotionData::NAME_SPECIAL_7);
#	ifdef ENABLE_WOLFMAN_CHARACTER
		s_kMap_stType_dwIndex.emplace("SPECIAL7", CRaceMotionData::NAME_SPECIAL_8);
#	endif
#endif
...
BOOL CRaceManager::GetRaceDataPointer(uint32_t dwRaceIndex, CRaceData** ppRaceData, bool printTrace)
#else
...
{
	m_kMap_iRaceKey_fRaceAdditionalHeight.emplace(std::map<int, float>::value_type(iVnum, fHeight));
}
