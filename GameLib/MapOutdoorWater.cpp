...
void CMapOutdoor::LoadWaterTexture()
{
	UnloadWaterTexture();
	char buf[256];
	for (int i = 0; i < WATER_INSTANCE_MAX_NUM; ++i)
	{
		sprintf(buf, "d:/ymir Work/special/water/%02d.dds", i + 1);
#ifdef ENABLE_WATER_OUTPUT_RENDER	//@custom011
		if (!CResourceManager::Instance().IsFileExist(buf)) {
			TraceError("Failed to load: %s", buf);
			continue;
		}
#endif

		m_WaterInstances[i].SetImagePointer(msl::inherit_cast<CGraphicImage *>(CResourceManager::Instance().GetResourcePointer(buf)));
	}
}

void CMapOutdoor::UnloadWaterTexture()
{
	for (auto & m_WaterInstance : m_WaterInstances)	//for (int i = 0; i < WATER_INSTANCE_MAX_NUM; ++i)
		m_WaterInstance.Destroy();
}
