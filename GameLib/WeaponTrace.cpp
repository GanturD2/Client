...
void CWeaponTrace::Update(float fReachScale)
{
	...
	if (m_isPlaying && m_fz >= 0.0001f)
	{
		...
		if (m_pInstance->GetCompositeBoneMatrix(m_dwModelInstanceIndex, m_iBoneIndex, &pMatrix))
		{
			...
			m_PDTVertexVector.emplace_back(PDTVertex);*/
...
bool CWeaponTrace::BuildVertex()
{
	...
	m_LongTimePointList.emplace_back(TTimePoint(0.00,D3DXVECTOR3(0,0,0)));
	m_LongTimePointList.emplace_back(TTimePoint(0.01,D3DXVECTOR3(0,1,0)));
	m_LongTimePointList.emplace_back(TTimePoint(0.04,D3DXVECTOR3(0,1,0)));
	m_LongTimePointList.emplace_back(TTimePoint(0.05,D3DXVECTOR3(0,0,0)));
...
void CWeaponTrace::Render()
{
	...
	STATEMANAGER.DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, int(m_PDTVertexVector.size() - 2), &m_PDTVertexVector[0], sizeof(TPDTVertex));
...
{
	auto * pImage = msl::inherit_cast<CGraphicImage *>(CResourceManager::Instance().GetResourcePointer("lot_ade10-2.tga"));
