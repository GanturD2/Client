...
void CArea::__UpdateLoadedObjectInstances(D3DXVECTOR3& v3Player)
{
	...
	const uint32_t dwStartTime = ELTimer_GetMSec();
	...
	for (auto it = m_ObjectInstanceVector.begin(); it != m_ObjectInstanceVector.end(); ++it, ++i)
	{
		...
		if (bDidModify)
		{
			const uint32_t dwElapsed = ELTimer_GetMSec() - dwStartTime;
			if (dwElapsed > 2)
				break;
		}
	}
...
{
	bool operator()(CEffectInstance * pkLeft, CEffectInstance * pkRight) { return pkLeft->LessRenderOrder(pkRight); }
};
...
void CArea::__SetObjectInstance_SetEffect(TObjectInstance * pObjectInstance, const TObjectData * c_pData, CProperty * pProperty)
{
	...
	D3DXMATRIX mat;
	D3DXMatrixRotationYawPitchRoll(&mat, D3DXToRadian(c_pData->m_fYaw), D3DXToRadian(c_pData->m_fPitch), D3DXToRadian(c_pData->m_fRoll));
...
#ifdef ENABLE_RENDERING_ONLY_IN_AREA_V2
	pObjectInstance->dwEffectInstanceIndex = (uint32_t)pEffectInstance;
#else
...
void CArea::__SetObjectInstance_SetTree(TObjectInstance * pObjectInstance, const TObjectData * c_pData, CProperty * pProperty)
{
	...
	pObjectInstance->SetTree(c_pData->Position.x, c_pData->Position.y, c_pData->Position.z + c_pData->m_fHeightBias, c_pData->dwCRC,
							 c_szTreeName);
}
...
void CArea::__SetObjectInstance_SetBuilding(TObjectInstance * pObjectInstance, const TObjectData * c_pData, CProperty * pProperty)
{
	...
	auto * pThing = msl::inherit_cast<CGraphicThing *>(rkResMgr.GetResourcePointer(Data.strFileName.c_str()));
	pThing->AddReference();
...
void CArea::__LoadAttribute(TObjectInstance * pObjectInstance, const char * c_szAttributeFileName)
{
	...
	auto * pAttributeData = msl::inherit_cast<CAttributeData *>(CResourceManager::Instance().GetResourcePointer(c_szAttributeFileName));
	...
	if (false == bFileExist)
	{
		...
		const bool bIsDungeonObject =
			(std::string::npos != attrFileName.find("/dungeon/")) || (std::string::npos != attrFileName.find("\\dungeon\\"));
...
bool CArea::__Load_LoadObject(const char * c_szFileName)
{
	...
	for (uint32_t i = 0; i < dwCount; ++i)
	{
		...
		if (rVector.size() > 6)
		{
			for (int portalIdx = 0; portalIdx < std::min<size_t>(rVector.size() - 6, PORTAL_ID_MAX_NUM); ++portalIdx)
...
bool CArea::__Load_LoadAmbience(const char * c_szFileName)
{
	...
	for (uint32_t i = 0; i < dwCount; ++i)
	{
		...
		if (!CPropertyManager::Instance().Get(ObjectData.dwCRC, &pProperty))
		{
			TraceError(" CArea::LoadAmbience Property(%d) FileName(%s) Load ERROR", ObjectData.dwCRC, c_szFileName);
			continue;
		}
