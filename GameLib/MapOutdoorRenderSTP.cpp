...
void CMapOutdoor::__SoftwareTransformPatch_RenderPatchSplat(SoftwareTransformPatch_SRenderState & rkTPRS, long patchnum,
															uint16_t wPrimitiveCount, D3DPRIMITIVETYPE ePrimitiveType, bool isFogEnable)
{
...
void CMapOutdoor::__SoftwareTransformPatch_RenderPatchNone(SoftwareTransformPatch_SRenderState & rkTPRS, long patchnum,
														   uint16_t wPrimitiveCount, D3DPRIMITIVETYPE ePrimitiveType)
{
	...
	auto * akPosition = (D3DXVECTOR4 *) akTransVertex;
...
bool CMapOutdoor::__SoftwareTransformPatch_SetTransform(SoftwareTransformPatch_SRenderState & rkTPRS,
														SoftwareTransformPatch_STLVertex * akTransVertex,
														CTerrainPatchProxy & rkTerrainPatchProxy, uint32_t uTerrainX, uint32_t uTerrainY,
...
bool CMapOutdoor::__SoftwareTransformPatch_SetSplatStream(SoftwareTransformPatch_STLVertex * akSrcVertex)
{
	...
	if (FAILED(pkVB->Lock(0, dwVBSize, (VOID**) &akDstVertex, 0) //D3DLOCK_DISCARD)
			   ))
		return false;
...
{
	{
		for (uint32_t uIndex = 0; uIndex != SoftwareTransformPatch_SData::SPLAT_VB_NUM; ++uIndex)
		{
			...
			if (FAILED(ms_lpd3dDevice->CreateVertexBuffer(sizeof(SoftwareTransformPatch_SSplatVertex) * CTerrainPatch::TERRAIN_VERTEX_COUNT,
														  ...
														  D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_SPECULAR | D3DFVF_TEX2, D3DPOOL_SYSTEMMEM,
...

	{
		for (uint32_t uIndex = 0; uIndex != SoftwareTransformPatch_SData::NONE_VB_NUM; ++uIndex)
		{
			...
			if (FAILED(ms_lpd3dDevice->CreateVertexBuffer(sizeof(SoftwareTransformPatch_STVertex) * CTerrainPatch::TERRAIN_VERTEX_COUNT,
														  D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFVF_XYZRHW, D3DPOOL_SYSTEMMEM,
