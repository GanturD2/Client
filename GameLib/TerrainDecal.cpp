...
void CTerrainDecal::Make(D3DXVECTOR3 v3Center, D3DXVECTOR3 v3Normal, D3DXVECTOR3 v3Tangent, float fWidth, float fHeight, float fDepth)
{
	...
	msl::refill(v3AffectedVertex);
	msl::refill(v3AffectedNormal);
...
void CTerrainDecal::SearchAffectedTerrainMesh(float fMinX, float fMaxX, float fMinY, float fMaxY, uint32_t * pdwAffectedPrimitiveCount,
											  D3DXVECTOR3 * pv3AffectedVertex, D3DXVECTOR3 * pv3AffectedNormal)
{
	if (!m_pMapOutdoor)
		return;
