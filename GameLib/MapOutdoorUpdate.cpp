...
struct FGetShadowReceiverFromCollisionData
{
	...
	FGetShadowReceiverFromCollisionData(CDynamicSphereInstance * pdsi, std::vector<CGraphicObjectInstance *> * pkVct_pkShadowReceiver)
...
struct FGetShadowReceiverFromHeightData
{
	...
	uint32_t GetCollectCount() { return m_dwCollectCount; }
...
struct PCBlocker_SInstanceList
{
	...
	uint32_t Size() { return m_dwBlockerCount; }
	...
	void operator()(CGraphicObjectInstance * pInstance)
	{
		...
		for (i = m_pkDSIVector->Begin(); i != m_pkDSIVector->End(); ++i)
		{
			...
			if (pInstance->CollisionDynamicSphere(rkDSI))
			{
				...
				{
					if ((msl::inherit_cast<CActorInstance *>(pInstance))->IsBuilding())
						__AppendObject(pInstance);
				}
			}
		}
...
void CMapOutdoor::UpdateAreaList(long lCenterX, long lCenterY)
{
	...
	FPushAreaToDeleteVector rPushAreaToDeleteVector =
		std::for_each(m_AreaVector.begin(), m_AreaVector.end(), FPushAreaToDeleteVector(eDeleteLRDir, eDeleteTBDir, m_CurCoordinate));
	...
	if (!rPushTerrainToDeleteVector.m_ReturnTerrainVector.empty())
	{
		...
		std::copy(rPushTerrainToDeleteVector.m_ReturnTerrainVector.begin(), rPushTerrainToDeleteVector.m_ReturnTerrainVector.end(),
