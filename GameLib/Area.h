...
public:
	...
	{
		bool operator()(TObjectData & lhs, TObjectData & rhs) const noexcept { return lhs.dwCRC < rhs.dwCRC; }
	};
	...
	typedef struct SAmbienceInstance : public CScreen
	{
		...
		SAmbienceInstance();
		virtual ~SAmbienceInstance() = default;
	...
	typedef struct SObjectInstance
	{
		...
		virtual ~SObjectInstance() = default;
	} TObjectInstance;
	...
	struct CRCNumComp
	{
		bool operator()(TCRCWithNumber & lhs, TCRCWithNumber & rhs) const { return lhs.dwNumber > rhs.dwNumber; }
	};
...
public:
	...
	void SetCoordinate(const uint16_t & usCoordX, const uint16_t & usCoordY);
