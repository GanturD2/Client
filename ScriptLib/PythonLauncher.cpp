...
int TraceFunc(PyObject * obj, PyFrameObject * f, int what, PyObject * arg)
{
	...
	case PyTrace_CALL:
		...
		_snprintf(szTraceBuffer, sizeof(szTraceBuffer), "Call: File \"%s\", line %d, in %s", PyString_AsString(f->f_code->co_filename),
				  f->f_lineno, funcname);
	...
	case PyTrace_EXCEPTION:
		...
		_snprintf(szTraceBuffer, sizeof(szTraceBuffer), "Exception: File \"%s\", line %d, in %s",
				  PyString_AS_STRING(f->f_code->co_filename), f->f_lineno, PyString_AS_STRING(f->f_code->co_name));
...
bool CPythonLauncher::RunCompiledFile(const char * c_szFileName)
{
	...
	magic = _PyMarshal_ReadLongFromFile(fPtr.get());
	...
	_PyMarshal_ReadLongFromFile(fPtr.get());
	v = _PyMarshal_ReadLastObjectFromFile(fPtr.get());
	fPtr.close();
...
bool CPythonLauncher::RunMemoryTextFile(const char * c_szFileName, uint32_t uFileSize, const VOID * c_pvFileData)
{
	NANOBEGIN
	...
	{
		for (uint32_t i = 0; i < uFileSize; ++i)
		{
...
bool CPythonLauncher::RunFile(const char * c_szFileName)
{
...

	{
		...
		acBufData.resize(dwBufSize);
		memcpy(acBufData.data(), pvData, acBufData.size());
	}

	bool ret = false;

	ret = RunMemoryTextFile(c_szFileName, dwBufSize, acBufData.data());
