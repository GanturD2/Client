...
static int r_short(RFILE * p)
{
	int16_t x;
	x = (int16_t) r_byte(p);
	x |= (int16_t) r_byte(p) << 8;
	/* Sign-extension, in case int16_t greater than 16 bits */
	x |= -(x & 0x8000);
	return x;
}
...
static PyObject * r_long64(RFILE * p)
{
	...
	unsigned char buf[8];
	int one = 1;
	auto is_little_endian = (int) *(char *) &one;
	if (is_little_endian)
	{
		memcpy(buf, &lo4, 4);
		memcpy(buf + 4, &hi4, 4);
...
static PyObject * r_object(RFILE * p)
{
	...
	case TYPE_LONG:
	{
		...
		ob->ob_size = n;
		for (i = 0; i < size; i++)
			ob->ob_digit[i] = (int16_t) r_short(p);
		return (PyObject *) ob;
	}
	...
	case TYPE_FLOAT:
	{
		...
		buf[n] = '\0';
		PyFPE_START_PROTECT("atof", return 0) dx = atof(buf);
		PyFPE_END_PROTECT(dx) return PyFloat_FromDouble(dx);
	}
	...
	case TYPE_COMPLEX:
	{
		...
		buf[n] = '\0';
		PyFPE_START_PROTECT("atof", return 0) c.real = atof(buf);
		PyFPE_END_PROTECT(c) n = r_byte(p);
		if (r_string(buf, (int) n, p) != n)
		{
			PyErr_SetString(PyExc_EOFError, "EOF read where object expected");
			return nullptr;
		...
		buf[n] = '\0';
		PyFPE_START_PROTECT("atof", return 0) c.imag = atof(buf);
		PyFPE_END_PROTECT(c) return PyComplex_FromCComplex(c);
	}
#endif
	...
	case TYPE_CODE:
		...
		else
		{
			...
			if (!PyErr_Occurred())
			{
				v = (PyObject *) PyCode_New(argcount, nlocals, stacksize, flags, code, consts, names, varnames, freevars, cellvars,
